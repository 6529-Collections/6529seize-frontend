...
Currently configured project: `6529seize-frontend` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/6529seize-frontend/SETUP.md` to fill in
   `data/6529seize-frontend/INFO.md`.
...
`--project-id 6529seize-frontend` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/6529seize-frontend/`.
