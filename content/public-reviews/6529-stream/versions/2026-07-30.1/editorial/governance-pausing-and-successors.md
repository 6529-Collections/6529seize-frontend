...
Stream keeps the permanent Core bytecode fixed at its original address.
Governance works through visible modules, scheduled actions, defined pause
domains, frozen powers, and explicit successor records.
...
duties can move to new, separately visible contracts. Every change and the
actor responsible for it remain available for inspection.
