...
Stream is an incomplete, predeployment, pre-audit candidate under active public
review. Its live contract count and funds held are both zero.
...
This page records the current path, connected foundations, source-only systems,
accepted design targets, open proposals, and the evidence required before
release. The topical pages explain why the mechanisms exist and how they
behave.

## Five implementation states

Implementation state and evidence are different questions. A contract may
compile and have strong local tests while remaining outside the candidate path
that users would actually call.

| State                    | Meaning                                                                                                                                            |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Current path**         | Constructed and connected by the current rehearsal in the user-facing path being described; currently in predeployment review.                     |
| **Connected foundation** | Constructed by the rehearsal and connected to selected protocol components outside the current user-facing sale or mint path.                      |
| **Source implemented**   | Solidity exists at the pinned commit; complete candidate wiring, addresses, grants, and configuration still require evidence.                     |
| **Accepted target**      | The architecture is accepted for pre-genesis work and awaits complete conforming source and integration evidence.                                 |
| **Proposed or deferred** | The material is open design work or intentionally outside current contract behavior.                                                              |
...
## Accepted targets awaiting implementation

Accepted targets have approved design direction and await conforming code and
...
| Core bytecode margin            | The pinned build proof records locally compiled `StreamCore` deployed-runtime bytecode at `24,152` bytes: `424` bytes below EIP-170. It passes the interim `384`-byte margin by `40` bytes but misses the normative `2,000`-byte margin by `1,576` bytes. | [Contract bytecode size](#contract-bytecode-size)                                                                                                                                                                                               |
...
disposition should link a source commit, test, or documented decision. Evidence
...
That sophistication increases the need for an exact readiness ledger. Review
should establish the real requirement protected by every mechanism, its bounded
authority, and independent reconstruction of the complete system.

Evidence is the standard for release.
