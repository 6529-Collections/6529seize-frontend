...
Art sales often need to pay several people: an artist, collaborator,
...
accounting systems: retain and fully specify the local native paths, or make
the resolver and settlement foundation the single supported route.

## Pull credits keep one recipient from blocking everyone

Calling every recipient during mint or settlement makes the entire transaction
depend on every recipient contract.
...
Pull accounting keeps credit and solvency state explicit. Each value-holding
contract must prove that credits plus reserves never exceed held value.

The current system uses several local credit ledgers. Fixed-price recipients
withdraw Drop-local credits, auction recipients and displaced bidders withdraw
Auction-local credits, and split-wallet recipients withdraw wallet-local
allocations after that wallet has been funded. Cross-contract solvency evidence
...
The resolver gives every supported sale one reusable source for precedence,
profile identity, split selection, and freeze semantics. Every supported sale
must call it.
...
ERC-20 contracts have varied transfer behavior. Exact return-value and balance
checks keep the recorded value equal to the value received and protect
recipient accounting from token-specific behavior.
...
2. `StreamPrimarySaleSettlement`, which resolves revenue, consumes the
   settlement key, routes value, and records the official settlement after the
   primary-sale adapter pulls the payer.
...
Stream implements ERC-2981 as a royalty display standard. Marketplace payment
remains voluntary.
...
Stream records the responsibilities needed to:

- apply the agreed split;
- retain enough funds for every claimant;
- handle recipients that reject a payment;
- reconcile refunds and remaining balances;
- preserve historical profiles;
- continue through key replacement and organizational change;
- describe the limits of secondary royalty information accurately.

Each obligation should have one accounting owner and enough public evidence for
artists, collaborators, curators, collectors, and auditors to reconstruct the
result.
...
- A curator root is valid and contains the wrong inputs.
