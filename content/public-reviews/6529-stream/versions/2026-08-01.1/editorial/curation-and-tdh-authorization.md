...
This division keeps community judgment flexible and makes the resulting action
precise. Artists and collectors should be able to see who signed the result,
which service prepared it, and exactly which terms the contract will execute.
...
Binding both supports sponsored or delegated purchases while fixing the
intended payer and artwork recipient.
...
The structure contains a quantity field. The current Drop validation requires
`quantity == 1`, so one consumed Drop authorization authorizes one token.
...
An edition can contain many tokens. This path needs a separate authorization
for each one. The UI should describe that actual execution path.
...
The receipt gives an artist, collector, auditor, or community member public
evidence for comparing the decision, signature, contract execution, and
explanation.

## What the public record should show

Stream keeps curation and TDH calculation in the community process. A configured
signer authorizes the result for contract execution. The public experience
should say that plainly and let people compare the community decision, signed
terms, transaction, and resulting receipt.

This structure supports evolving community rules while keeping every approved
contract term fixed during execution.
...
- auction registration and later signer cancellation produce inconsistent
  expectations;
- a failed execution consumes an authorization and leaves the intended action
  incomplete.
