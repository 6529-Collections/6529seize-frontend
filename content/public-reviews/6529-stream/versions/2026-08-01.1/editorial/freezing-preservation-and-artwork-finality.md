...
2. **Core freeze** — selected collection configuration in the permanent token
   contract becomes permanently fixed.
...
Preservation records remain appendable after Core freeze. Every old record
remains available. A later record can update the collection/type/subject
`latest` pointer. `Latest` means most recently recorded
onchain, independent of `effectiveAt`.

This allows future conservators to add a new storage location, fixity check, or
format note while preserving the original final package.
...
Record-family authorization exists in source. Complete candidate evidence for
admitted types, provider addresses, grants, runtime code hashes,
...
Each step should produce a machine-readable receipt. The UI should explain the
irreversible effects and present the relevant calldata in ordinary language.
...
The finality process should let artists, collectors, and independent reviewers
verify:

- every mint path is closed;
- every remaining mutable metadata field is identified;
- each hash covers a canonical encoding;
- token-specific artifacts are included;
- required bytes remain retrievable;
- the published runtime can execute them;
- replacement renderers preserve the intended work;
- the artist approved the exact final state;
- every administrative mutation path has ended.

Each layer has one precise meaning and contributes evidence to the artwork's
permanent record.
...
9. What receipt lets an artist and collector verify the terminal state from
