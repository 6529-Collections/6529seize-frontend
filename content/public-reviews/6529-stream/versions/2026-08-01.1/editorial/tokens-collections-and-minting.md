...
All Stream works share one permanent ERC-721 Core for ownership, approvals,
transfers, identity reads, and common rules. Public Core records identify each
artist's collection within that shared contract.

The shared Core gives many collections the same foundation and risk surface. A
Core defect or control failure can affect many collections. Each artist's
collection uses a Core record within the shared ERC-721 contract. Address-only
marketplaces may group every Stream artist and collection under one contract.
...
Separate ERC-721 facade addresses are outside the launch design. See the accepted
...
Stream tracks live supply, lifetime minted supply, permanent token allocation,
and the remaining capacity of each sale or authorization separately. These
records preserve the correct limits after burns, minter replacement, and use of
...
This is purposeful modularity. Distribution rules can evolve while existing
token identity stays fixed. The security condition is that no module
...
execution. The rehearsal installs the manager in Core. `StreamDrops` receives
the legacy minter.
...
their own timing and accounting while the ERC-721 Core remains stable.
...
`StreamMintLedger` keeps counter history through transactions and manager
replacement. Users can verify that each limit is applied consistently and that
a module change preserves the policy's earlier usage.
...
`DropAuthorization` contains `quantity`.
...
An edition can contain many tokens. The current signed Drop lane needs a
separate authorization for each one. The manager lane has different
...
The manager, ledger, gates, operation identities, and collection records keep
the following rules public and durable:
...
For a protocol intended to carry valuable artwork over decades, these records
make supply and minting promises independently verifiable. Each rule should have
one clear owner and the smallest implementation that preserves it.
