...
Stream keeps the permanent Core at its original address. Other service
contracts can change through public, delayed, and recorded decisions. Emergency
powers apply to specific operations, and selected powers can end permanently.

This page explains how Stream changes over time. [Who Can Do
What](./roles-and-trust) explains who may perform each action.
...
Bootstrap sealing then closes this setup authority permanently.
...
registration can bind runtime code hash and interface expectations. Clients can
require those registered values, and every change leaves evidence.
...
- how clients discover the current module from a public registry.

A matching code hash proves which runtime bytecode occupied an address at the
measured time. Separate review establishes safety, initialization, and the
...
Execution must use that exact record, including the recipient, value, address,
and calldata visible during review.
...
The mechanism exists. The readiness record must still prove complete binding
for every governed selector in a concrete candidate.
...
Numeric IDs are append-only. Former class `6` is permanently retired.
...
- recovering funds requires a precise proof that the amount excludes
  liabilities;
...
configuration, payable calls, unaccounted balances,
or interact with targets in ways visible only through complete calldata and
target analysis.
...
- how a tightly limited action recovers residual ETH after proving liabilities.

This boundary belongs in both code and accounting.
...
- the new value must be strictly higher and at most 2x the current value;
- host values move upward only through this delayed route;
- zero governance authority makes the host immutable;
- permanent governance loss makes the values read-only;
- tightening later requires a reviewed successor host or deployment line.

This policy keeps the authority surface small and auditable. The source hosts
implement the rule. The complete candidate parameter
...
The guardian is a rapid defensive role that can stop or veto a defined
operation, particularly during a timelock.
...
normally require the full governance path. A guardian veto stops an
already-proposed payload; a different payload requires a new governance action.
...
window. Continuous monitoring makes the veto power effective.
...
| Domain             | Stops                                                                                               | Continuing operations                                                      |
| ------------------ | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
...
`StreamMintManager` has a separate per-phase pause that operates independently
of the global `MINT` domain. Any cutover must decide whether that distinction is
...
The final freeze evidence should enumerate every executor, privileged module,
direct route, and equivalent mutation path that can perform the same write, then
prove each path is closed.

## Replacing a signing key

Signed authorizations are tied to signer epochs. Rotation invalidates prior
authorizations and gives the new signer a distinct epoch.
...
Epochs make a change visible. Operational evidence must also establish control
of the new signer and application of the community's policy.

## Replacing a service contract

A successor is a new immutable contract recognized as following an older
module. The predecessor, its bytecode, and its history remain onchain.
Governance records the transition as a new relationship between immutable
contracts.
...
A successor should preserve token history and explicit artwork commitments. If
it changes what bytes are served, who can mint, how funds move, or
...
pause, veto, rotate, or withdraw. A credible security model names those exact
powers and their constraints.

Emergency recovery safeguards must prevent:
...
assumptions change, and some modules will need successors. Explicit governance
keeps those changes in recorded coordination and bounded authority.

The governance design therefore separates:

- the permanent identity that remains fixed;
- replaceable duties that may need a successor;
- temporary intervention limited to stopping immediate harm;
- delayed decisions whose complete effects should be inspectable in advance;
- powers that can be surrendered permanently.

The standard for simplification is preservation of the same artistic,
...
- a guardian authors a replacement beyond its veto authority;
...
- a predecessor and successor accept the same authorization or spend the same
  liability;
- clients follow a new module from an unverified continuity record.

## Questions for reviewers

1. Does the list show every action each role can take?
2. Should the contract that applies scheduled changes ever be able to send ETH?
...
7. Is 30 days enough notice for a successor that changes an artwork-affecting
   duty?
8. What is the smallest set of change controls that preserves public notice,
   emergency response, and recorded coordination?
