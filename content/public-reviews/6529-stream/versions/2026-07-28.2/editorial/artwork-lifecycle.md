...
11. The longer-term source design can record a visible successor for
    replaceable infrastructure without rewriting the token's Core history; the
    current rehearsal does not yet deploy or prove that cutover machinery.
...
receives both a deployment-global ID and a collection-local serial. That lets
...
The proposed first Network program uses a threshold rather than a ranking. Its
tentative values and unresolved auction behavior are centralized in
[the proposed policy](./overview#the-intended-first-6529-network-use-case) and
[the readiness ledger](./security-testing-and-known-limitations#product-choices-still-open).
For this lifecycle, the important boundary is that the threshold process
happens offchain and produces a signed authorization for the onchain auction.
...
The Core allocates a deployment-global token ID, associates it with the
collection, and records its collection-local serial. Supply is tracked as
minted-ever history, not merely as the number of tokens currently held.
