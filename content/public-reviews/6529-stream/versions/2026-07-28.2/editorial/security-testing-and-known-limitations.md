...
These are 6529 Network product and deployment decisions. They are not current
Solidity behavior and must be resolved before the public launch configuration
can be called final.

| Question | Current position |
| --- | --- |
| Main Stage name and scope | Working name for a proposed threshold-based 1/1 program; not final |
| TDH policy | 10 million TDH held for 24 hours is a candidate rule calculated and evidenced offchain |
| Auction policy | A zero-reserve auction initially scheduled for 24 hours is proposed. The pinned source permits zero-value bids at zero reserve, and its unsigned contract-wide extension setting can change during an active auction. Both behaviors require an explicit launch decision. |
| Primary-sale proceeds | 75% artist and 25% Network is intended; the reviewed source defaults to 50% poster / 25% protocol / 25% curator, a separate target specification says 90% artist / 10% protocol, and neither hardcodes the new policy. Both sale lanes and the poster-to-artist and protocol-to-Network recipient bindings need candidate proof. Auction settlement also reads a mutable split that is not signed or snapshotted at registration, so that configuration must be pinned or protected from drift through settlement. |
| Museum accession collections | Open choice between native collections in the main Stream and a separate Museum deployment |
| Other creator or community programs | Open choice between another native collection in the shared Core and an independent deployment |
| Independent-deployment discovery | An onchain registry is an open design question; registration would not imply shared governance, audit status, or canonical endorsement |
| Launch authority and evidence | Signer, authorization builder, TDH evidence, parameter-change process, and candidate-bound configuration remain to be published |

Every contract and readiness statement below refers to exact Git commit
