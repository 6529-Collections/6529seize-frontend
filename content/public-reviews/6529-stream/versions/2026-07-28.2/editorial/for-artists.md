...
record and its artist address. Each token receives both a deployment-global
Stream token ID and a collection-local serial.
...
The [proposed Main Stage
policy](./overview#the-intended-first-6529-network-use-case) is tentative.
Before participating, an artist would need the published evidence standard,
signer identity, exact auction behavior, and candidate proof that both sale
lanes pay the intended percentages to the intended artist and Network
addresses—not merely to accounts carrying the contract's `poster` and
`protocol` labels.
