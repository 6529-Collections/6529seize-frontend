...
Consider a five-work edition. The first successful mint receives a
deployment-global Stream token ID and serial `1` inside the artist’s
collection. Later mints receive new deployment-global IDs and collection
serials `2` through `5`. If token `2` is burned, its ownership ends, but its
ID, serial, and mint history are not reused. When the collection closes, no
sale path or replacement minter should be able to create a sixth work.
...
contract. Token IDs are globally sequential inside that deployment. Core also
records a collection-local serial, so an artwork can be identified both as a
deployment-global Stream token and as an item within its artist collection.

For the proposed 6529 program, this is what makes an artist **native
collection** possible—the product may provisionally describe it as a
“subcollection.” A photographer can create one coherent native collection in
the main Core while each individual photograph still needs its own one-use
authorization from the configured signer after the offchain community process.
...
A separate Stream deployment is a different topology, not another native
collection within the same Core. It would have its own Core address,
deployment-global ID
sequence, roles, parameters, evidence, and governance. Whether related
deployments should publish themselves through an onchain discovery registry is
an open product question; the reviewed Core does not make them one collection
or one trust domain.
...
2. Are deployment-global token IDs plus collection-local serials the right
