...
## The proposed 6529 Network policy

The first Network use case under discussion is a threshold program rather than
a ranked contest. A work in the provisionally named **Stream Main Stage** would
advance after remaining above a published TDH threshold for a published
interval. **10 million TDH** and **24 hours above threshold** are candidate
parameters, not current Network policy and not values enforced by this
contract.

After the offchain process determines and records that the published rule was
satisfied, an authorization builder should prepare the standard auction
payload, the configured signer should sign it, and a caller should submit it.
The intended policy can be rule-driven, but it is not autonomous Solidity.
Stream verifies the signer and payload; it cannot prove the TDH calculation,
threshold interval, evidence, or behavior of those offchain actors.

This proposed threshold interval is also distinct from the proposed
**24-hour scheduled auction**. The first is a curation-policy question. The
second becomes the initial `auctionEndTime` in the signed authorization. The
pinned source's contract-wide extension setting is not signed or pinned per
auction and may be changed by an authorized admin while an auction is active.
