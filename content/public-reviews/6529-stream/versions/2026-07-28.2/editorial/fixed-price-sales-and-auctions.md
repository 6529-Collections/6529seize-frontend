...
## Where the proposed Main Stage hands off

The proposed 6529 Network policy would do its TDH counting and threshold timing
before this page's contract flow begins. Once the offchain rule is satisfied, a
configured signer can authorize a zero-reserve English auction with an exact
end time.

A 24-hour scheduled auction is a product proposal, not a duration hardcoded by
Stream. The authorization carries an absolute `auctionEndTime`; the auction
contract requires that time to be in the future. Late bids can extend it using
the then-current contract-wide `extensionTime`. In the pinned source, an
authorized admin can change that value during an active auction; it is not
signed or pinned per auction. “24-hour auction” therefore describes the
proposed initial window, not a fixed actual closing time.

Zero reserve also needs an explicit product decision. In the pinned source, a
zero-reserve auction accepts a zero-ETH first bid. Because another zero bid is
again treated as an initial bid, it can replace the earlier zero bidder without
a positive increment. The launch policy must decide whether a zero-price
winning bid is intended; otherwise the auction contract needs a different
minimum-bid rule.
