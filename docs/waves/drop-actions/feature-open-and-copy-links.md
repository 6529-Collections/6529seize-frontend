...
- Desktop wave-drop action controls (`More` menu)
...
- Use `Open drop` from desktop `More` or the long-press/touch action menu on
  non-hover devices.
- Use `Copy link` from desktop `More` or the long-press/touch action menu on
  non-hover devices.
