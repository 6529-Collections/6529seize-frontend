...
- Environment notes for other threads:
  ...
  - knip reported ~23 false unused-file/export findings (`ops/scripts/*`
    files, gate-script exports) from worktrees nested under a hidden tool-state
    directory; identical content analyzed cleanly from a normal sibling
    worktree or the main checkout.
    - SOLVED 2026-07-05 (root cause via `knip --debug` A/B at e70de6a90,
      nested worktree versus a short-path control, with identical
      junctioned node_modules). Mechanism: the root `.gitignore` had an
      unanchored tool-state ignore line; knip converts root-gitignore rules
      into broad picomatch ignores
      ...
      the hidden worktree parent, each absolute path contained the ignored
      segment, so fast-glob dropped it. 28 files fell out of the graph
      ...
      members as UNUSED EXPORT. PR #3121 anchored the ignore rule to the
      repository root. Verified clean (exit 0, zero findings) in
      both the nested worktree and the short-path control at the same
      commit; `git check-ignore` confirmed the intended root directory remained
      ignored. Caveats: checkouts of commits predating the fix
...
- Wave 1 launched the merge-gate and branch-amnesty lanes; cross-session state
  was preserved in the tracked campaign records.
...
- Phase 1 (rescue): a 117-status-entry dirty tree was preserved on a rescue
  branch. A credential-bearing session artifact was detected, removed from the
  rewritten branch, and handled through the private security process. This
  public archive intentionally omits credential, machine-local, and current
  remediation details.
...
- A separate PR-followup worktree (on `1a-staging`, with untracked
  release-check tests) intentionally untouched — plausibly the external
  release-tender thread's working dir.
- Unrelated processes in a different repository were left alone.
...
- Staging: main merged to 1a-staging → 8fced9f36 deployed green. Authenticated
  staging access was supplied without retaining machine-local credential
  details. The 12-pack
...
- Delegation any-tail slice 2 of 3: NewConsolidation, NewSubDelegation,
  ...
  unknown/Error | null). Mechanical edits followed a strict per-line spec and
  were reviewed hunk-by-hunk before commit.
  `any_casts` 45 -> 21.
