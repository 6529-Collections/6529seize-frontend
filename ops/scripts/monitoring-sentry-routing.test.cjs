...
const canonical = value => Array.isArray(value) ? value.map(canonical) : value !== null && typeof value === 'object' ?
  Object.fromEntries(Object.keys(value).sort(lexical).map(key => [key, canonical(value[key])])) : value;
...
function fixtures() {
  ...
  KEY_ORDER.forEach((key, index) => {
    const back = key.startsWith('back');
    const type = key.endsWith('Email') ? 'email' : key.endsWith('Discord') ? 'discord' : 'webhook';
    workflows[key] = {
      ...
      enabled: true, config: { frequency: key.endsWith('Email') ? 30 : key === 'backDiscord' ? 10 : 5 },
      environment: key === 'frontApp' ? 'production' : null, owner: { type: 'team', id: '900', name: CANARY },
      detectorIds: [back ? '100' : '200'],
      triggers: { id: String(300 + index), organizationId: '500', logicType: key === 'frontDiscord' ? 'all' : 'any-short',
        conditions: [{ id: String(400 + index), type: key.endsWith('Email') ? 'new_high_priority_issue' :
          key === 'backDiscord' ? 'first_seen_event' : 'event_frequency_count',
        comparison: key.endsWith('Email') || key === 'backDiscord' ? true : { value: 10, interval: '1m' }, conditionResult: true }], actions: [] },
      actionFilters: [{ id: String(500 + index), organizationId: '500', logicType: 'all', conditions: [],
        actions: [{ id: String(600 + index), type, integrationId: type === 'discord' ? '800' : null,
          data: type === 'email' ? { fallthroughType: 'ActiveMembers' } : {},
          config: { targetType: type === 'email' ? 'issue_owners' : 'specific', targetDisplay: CANARY,
            targetIdentifier: type === 'webhook' ? app.slug : type === 'discord' ? 'channel-private' : null }, status: 'active' }] }],
    };
  });
  const detectors = Object.fromEntries(['seize-backend', '6529-frontend'].map(project => {
    const back = project === 'seize-backend'; const id = back ? '100' : '200';
  ...
  const pins = { audit: actualPins.audit, app: hash(app.uuid), appConfig: fingerprint(appConfig(app)), callback: hash(app.webhookUrl),
    ...
    workflows: Object.fromEntries(KEY_ORDER.map(key => [key, [hash(workflows[key].id), configHash(workflows[key]), key.startsWith('back') ? 'seize-backend' : '6529-frontend']])),
    detectors: { 'seize-backend': hash('100'), '6529-frontend': hash('200') } };
...
async function execute(options = {}) {
  ...
  const context = { createHash, process, Buffer, URL, Object, Date: options.Date ?? Date,
    ...
    fetch: async (url, init) => {
      ...
      const call = { path, method: init.method };
      requests.push(call);
      assert.ok(requests.length <= 48);
      const override = options.onRequest?.({ state, path, init, requests, writes, snapshots });
      if (override?.throw) throw new Error(CANARY);
      let data; let status = 200; let link = null;
      if (init.method === 'PUT') {
        const body = JSON.parse(init.body); writes.push({ path, body: clone(body) });
        assert.equal(init.headers['Content-Type'], 'application/json');
        if (path === ORG + 'workflows/4/') {
          ...
          const donor = env.ROUTING_MODE === 'restore' ? baseline.frontDiscord : baseline.frontApp;
          ...
          if (env.ROUTING_MODE === 'restore') assert.equal(state.workflows.backDiscord.enabled, true);
          ...
          data = state.workflows.frontDiscord;
        } else if (path === ORG + 'workflows/?id=2') {
          assert.deepEqual(body, { enabled: env.ROUTING_MODE === 'restore' });
          ...
          data = [state.workflows.backDiscord];
        } else assert.fail('unexpected provider write path');
        if (options.denyWrite || (options.denyBackend && path === ORG + 'workflows/?id=2')) { status = options.denyStatus ?? 403; data = { detail: CANARY }; }
        ...
        if (path === LIST) { snapshots++; options.onSnapshot?.({ state, snapshots }); data = Object.values(state.workflows); }
        else if (/^organizations\/seize-ff\/workflows\/[1-6]\/$/.test(path)) data = Object.values(state.workflows).find(value => path.endsWith('/' + value.id + '/'));
        else if (path === ORG + 'detectors/100/' || path === ORG + 'detectors/200/') data = state.detectors[path.includes('/100/') ? '100' : '200'];
        else if (path === 'projects/seize-ff/seize-backend/') data = { id: '10', slug: 'seize-backend', access: ['project:read'] };
        else if (path === 'projects/seize-ff/6529-frontend/') data = { id: '20', slug: '6529-frontend', access: ['project:read'] };
        else if (path === ORG + 'sentry-apps/') data = [state.app];
        else if (path === ORG + 'sentry-app-installations/') data = [state.installed];
        else if (options.extraGet) data = options.extraGet(path, state);
        else assert.fail('unexpected provider GET path');
      }
      if (override?.data !== undefined) data = override.data;
      if (override?.status !== undefined) status = override.status;
      if (override?.link !== undefined) link = override.link;
      const bytes = Buffer.from(override?.raw ?? (init.method === 'PUT' && status >= 400 && options.deniedBody !== undefined ? options.deniedBody : JSON.stringify(data)));
      let sent = false;
      return { status, headers: { get: key => key === 'link' ? link : key === 'content-length' ? (override?.declared ?? String(bytes.length)) : null },
        body: override?.noBody ? null : { getReader: () => ({ read: async () => { if (sent) return { done: true }; sent = true;
          return { done: false, value: bytes }; }, cancel: async () => {} }) } };
    },
  };
...
test('already-applied frontend continues with only the backend write', async () => {
  ...
  assert.equal(value.code, 0); assert.equal(value.writes.length, 1); assert.equal(value.writes[0].path, ORG + 'workflows/?id=2');
});
...
for (const [label, override] of [
  ...
  ['terminal changed project', { link: '<' + API + LIST.replace('seize-backend', 'other') + '&cursor=end>; rel="next"; results="false"; cursor="end"' }],
...
test('list/detail configuration disagreement stops before mutation', async () => {
  const value = await execute({ onRequest: ({ path, state }) => path === ORG + 'workflows/4/' ? {
...
test('manual restore re-enables and verifies backend before restoring the original frontend action', async () => {
  ...
  assert.deepEqual(value.writes.map(write => write.path), [ORG + 'workflows/?id=2', ORG + 'workflows/4/']);
...
test('manual restore continues a valid partial restoration with only the frontend write', async () => {
  ...
  assert.equal(value.code, 0); assert.equal(value.writes.length, 1); assert.equal(value.writes[0].path, ORG + 'workflows/4/');
...
test('failed initial verification permits one reconciliation GET but no subsequent PUT', async () => {
  ...
  const value = await execute({ onRequest: ({ path, init, writes }) => {
    if (!injected && init.method === 'GET' && writes.length === 1 && path === ORG + 'workflows/4/') {
  ...
  assert.equal(value.result.results[0].status, 'observed_desired_after_uncertainty');
  assert.equal(value.requests.filter(call => call.method === 'GET' && call.path === ORG + 'workflows/4/').length, 3);
});
