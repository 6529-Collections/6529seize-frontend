...
while true; do
  ...
  rm -rf "$worktree_path/ops/docs"
  mkdir -p "$worktree_path/ops"
  ln -s "$REPO_ROOT/ops/docs" "$worktree_path/ops/docs"

  printf 'Running docs updater in worktree scope...\n'
  (cd "$worktree_path" && ./ops/docs/update_docs.sh)

  git -C "$REPO_ROOT" worktree remove -f "$worktree_path"
  ACTIVE_WORKTREE_PATH=""

  git -C "$REPO_ROOT" add ops/docs
