...
def find_route_matches(
    ...
    result = run(
        ["rg", "-l", "--fixed-strings", "--glob", "!ops/docs/**", token, *source_dirs],
        repo_root,
    )
...
def audit_area(repo_root: Path, docs_root: Path, area: str) -> AreaAudit:
    ...
    audit.merge_candidates.extend(compute_merge_candidates(f"ops/docs/{area}", root_features))
    ...
    for subarea_dir in subareas:
        ...
        scope = f"ops/docs/{area}/{subarea_dir.name}"
...
def print_pick(audit: AreaAudit, max_items: int) -> None:
    ...
    print("")
    print("next_commands:")
    print(f"- python3 ops/scripts/docs-area-remediator-local/area_health.py --area {audit.area}")
    print(
        "- python3 ops/scripts/docs-area-remediator-local/validate_docs_optimizations.py "
        f"--area {audit.area} --strict"
    )
    print("- python3 ops/scripts/docs-area-remediator-local/validate_docs_links.py")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--area", help="Area name under ops/docs/ (for example: waves)")
    parser.add_argument(
        "--pick",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not docs_root.exists():
        print("ERROR: ops/docs/ directory not found.", file=sys.stderr)
        return 1
