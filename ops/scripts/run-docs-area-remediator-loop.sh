...
META_FILE="$REPO_ROOT/ops/docs/meta.json"
LOCAL_DOCS_VALIDATOR="$REPO_ROOT/ops/scripts/docs-area-remediator-local/validate_docs_optimizations.py"
SKILL_DOCS_VALIDATOR="$REPO_ROOT/.codex/skills/docs-area-remediator/scripts/validate_docs_optimizations.py"
LOCAL_STALE_ROUTE_REMEDIATOR="$REPO_ROOT/ops/scripts/docs-area-remediator-local/run_stale_route_remediation.py"
SKILL_STALE_ROUTE_REMEDIATOR="$REPO_ROOT/.codex/skills/docs-area-remediator/scripts/run_stale_route_remediation.py"
LOCAL_DOCS_LINK_VALIDATOR="$REPO_ROOT/ops/scripts/docs-area-remediator-local/validate_docs_links.py"
SKILL_DOCS_LINK_VALIDATOR="$REPO_ROOT/ops/skills/commit-docs-updater/scripts/validate_docs_links.py"
...
1. Run \`python3 ops/scripts/docs-area-remediator-local/validate_docs_optimizations.py --all --strict\`.
...
4. Then remediate docs ownership sections in \`ops/docs/**\` only for routes still unresolved after parser fixes.
...
Validator failed on monotonic-quality for ops/docs/${target_area} because the area score regressed.
Primary objective: reduce ops/docs/${target_area} quality score by resolving signals that increased it.
If merge/split candidates exist, merge or restructure micro-pages when they represent a single user journey.
Keep scope local to ops/docs/${target_area} and related indexes, and keep links coherent.
EOF
  )
...
Target file: ${target_path}
Run in strict stateless mode: do not rely on previous or next iterations.
Perform a second-view refinement pass focused on monotonic-quality recovery for ops/docs/${target_area}.
...
pick_next_target() {
  ...
  done < <(find ops/docs -type f -name '*.md' | sort)
...
while true; do
  ...
  if ! target_path="$(pick_next_target "${last_target}")"; then
    echo "No docs markdown files found under ops/docs/. Stopping loop."
    break
  fi
  ...
  target_area=""
  if [[ "${target_path}" == "ops/docs/README.md" ]]; then
    is_root_target=true
  else
    target_area="${target_path#ops/docs/}"
...
EOF
  ...
  if [[ "${is_root_target}" == "true" ]]; then
    ...
    git add -A "ops/docs/${target_area}" ops/docs/README.md
  fi
  ...
  if [[ "${is_root_target}" == "true" ]]; then
    ...
    if ! area_validation_output="$(python3 "$DOCS_VALIDATOR" --area "${target_area}" --strict --enforce-monotonic 2>&1)"; then
      ...
      if printf '%s\n' "${area_validation_output}" | grep -q "stale-route signals"; then
        echo ""
        echo "Detected stale-route backlog in ops/docs/${target_area}; running auto-remediation."
        ...
        echo ""
        echo "Detected monotonic-quality regression in ops/docs/${target_area}; running auto-remediation."
        run_monotonic_recovery_pass "${target_path}" "${target_area}"
        git add -A "ops/docs/${target_area}" ops/docs/README.md
        python3 "$DOCS_VALIDATOR" --area "${target_area}" --strict --enforce-monotonic
      else
        echo ""
        echo "Area validation failed for ops/docs/${target_area}."
        exit 1
      fi
    else
      printf '%s\n' "${area_validation_output}"
  ...
  git commit -s -m "docs: remediator pass ${iteration} (${target_path})"
  write_last_target_to_meta "${target_path}"
  last_target="${target_path}"
done
