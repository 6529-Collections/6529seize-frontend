...
5. Deploy frontend production through the repo-approved path from `main` only. Current frontend production deploy is `Web Deploy - PROD` in `.github/workflows/build-upload-deploy-prod.yml`; `bin/ghdeploy` may be used only from a clean, upstream-synced `main` worktree. Never deploy production from feature branches, release branches, PR heads, tags, local WIP refs, or unmerged commits.
