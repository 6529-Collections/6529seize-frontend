...
describe("Security Logger", () => {
  describe("logSecurityEvent", () => {
    it("should not log when security logging is disabled", () => {
      withMockedEnv(
        { NODE_ENV: "development", ENABLE_SECURITY_LOGGING: "false" },
        () => {
          jest.isolateModules(() => {
            const securityLogger = require("@/src/utils/security-logger");
            const { createConnectionEventContext, logSecurityEvent } =
              securityLogger;
    ...
    it("should log when security logging is enabled in development", () => {
      withMockedEnv(
        { NODE_ENV: "development", ENABLE_SECURITY_LOGGING: "true" },
        () => {
          // IMPORTANT: re-import the module UNDER TEST here
          jest.isolateModules(() => {
            const securityLogger = require("@/src/utils/security-logger");
            const { createConnectionEventContext, logSecurityEvent } =
              securityLogger;
          ...
          // Now assert on the spies
          expect(mockConsoleLog).toHaveBeenCalledWith("i am logSecurityEvent");
          ...
          expect(mockConsoleWarn).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
              eventType: expect.any(String), // or SecurityEventType.WALLET_CONNECTION_ATTEMPT
              source: "test-source",
    ...
    it("should never log in production even when enabled", () => {
      withMockedEnv(
        { NODE_ENV: "production", ENABLE_SECURITY_LOGGING: "true" },
        () => {
          jest.isolateModules(() => {
            const securityLogger = require("@/src/utils/security-logger");
            const { createConnectionEventContext, logSecurityEvent } =
  ...
  describe("logError", () => {
    ...
    it("should include stack trace in development but sanitized", () => {
      withMockedEnv({ NODE_ENV: "development" }, () => {
        jest.isolateModules(() => {
          const securityLogger = require("@/src/utils/security-logger");
          const { logError } = securityLogger;
    ...
    it("should not include stack trace in production", () => {
      withMockedEnv({ NODE_ENV: "production" }, () => {
        jest.isolateModules(() => {
          const securityLogger = require("@/src/utils/security-logger");
          const { logError } = securityLogger;
  ...
  describe("createConnectionEventContext", () => {
    it("should create context with required fields only", () => {
      jest.isolateModules(() => {
        const securityLogger = require("@/src/utils/security-logger");
        const { createConnectionEventContext } = securityLogger;
  ...
  describe("createValidationEventContext", () => {
    it("should create context with validation fields", () => {
      jest.isolateModules(() => {
        const securityLogger = require("@/src/utils/security-logger");
        const { createValidationEventContext } = securityLogger;
  ...
  describe("createErrorEventContext", () => {
    it("should create context with error code", () => {
      jest.isolateModules(() => {
        const securityLogger = require("@/src/utils/security-logger");
        const { createErrorEventContext } = securityLogger;
    ...
    it("should sanitize multiple addresses in same message", () => {
      jest.isolateModules(() => {
        ...
        expect(mockConsoleError).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            message:
              "Transfer from 0x***REDACTED*** to 0x***REDACTED*** failed",
          })
        );
      });
    });
  ...
  describe("Environment-specific behavior", () => {
        ...
        withMockedEnv(
          ...
          () => {
            jest.isolateModules(() => {
              ...
              const context = createConnectionEventContext(
              ...
              expect(mockConsoleWarn).toHaveBeenCalledWith(
                "[SEIZE_SECURITY_EVENT]",
                expect.objectContaining({ userAgent: "server-side" })
              );
            });
            (global as any).navigator = originalNavigator;
          }
        ...
        jest.isolateModules(() => {
          ...
          expect(
            mockConsoleError.mock.calls[
              mockConsoleError.mock.calls.length - 1
            ][1]
    ...
    it("should handle all context creation functions server-side", () => {
      jest.isolateModules(() => {
        ...
        const errorContext = createErrorEventContext("server-test");
      ...
      withMockedEnv(
        ...
        () => {
          jest.isolateModules(() => {
            ...
            const maliciousContext = {
              ...createConnectionEventContext("test-source"),
              partialAddress: "0x742d35Cc6634C0532925a3b8D362Ad5C",
            };
            expect(() => {
              logSecurityEvent(
                SecurityEventType.WALLET_CONNECTION_ATTEMPT,
    ...
    it("should allow safe diagnostic data", () => {
      withMockedEnv(
        ...
        () => {
          jest.isolateModules(() => {
            const securityLogger = require("@/src/utils/security-logger");
            const { createValidationEventContext, logSecurityEvent } =
              securityLogger;
      ...
      withMockedEnv(
        ...
        () => {
          jest.isolateModules(() => {
            ...
            const maliciousContext = {
              ...createConnectionEventContext("test-source"),
              data: "0x742d35cc6634c0532925a3b8d362ad5c32b8b73d",
            };
            expect(() => {
              logSecurityEvent(
                SecurityEventType.WALLET_CONNECTION_ATTEMPT,
    ...
    it("should log AUTH_CLEANUP_FAILURE events", () => {
      withMockedEnv(
        ...
        () => {
          jest.isolateModules(() => {
            const securityLogger = require("@/src/utils/security-logger");
            const { createErrorEventContext, logSecurityEvent } =
              securityLogger;

            const context = createErrorEventContext(
              "auth-cleanup",
              "CLEANUP_FAILED"
            );
    ...
    it("should create valid ISO timestamps in all context creators", () => {
      jest.isolateModules(() => {
        ...
        const connectionContext =
          createConnectionEventContext("timestamp-test");
        const validationContext = createValidationEventContext(
          "timestamp-test",
          true
        );
