...
describe('commonApi utility methods', () => {
  it('commonApiPut posts JSON body', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: async () => ({ ok: 1 }) });
    const res = await commonApiPut({ endpoint: 'e', body: { a: 1 } });
    expect(res).toEqual({ ok: 1 });
    expect(global.fetch).toHaveBeenCalledWith('http://api/api/e', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'x-6529-auth': 'stage', Authorization: 'Bearer jwt' },
  ...
  it('commonApiPostForm posts form data without content-type', async () => {
    ...
    (global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: async () => ({ res: 3 }) });
    ...
    expect(global.fetch).toHaveBeenCalledWith('http://api/api/f', {
      method: 'POST',
