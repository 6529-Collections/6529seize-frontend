...
const { getRole, validateJwt } = jwtValidationUtils;
import {
  getRefreshToken,
  getWalletAddress,
  getWalletRole,
...
jest.mock("@/services/auth/session-v2.utils", () => ({
  isLegacyRefreshEnabled: jest.fn(() => true),
...
const mockedSyncWalletRoleWithServer =
  syncWalletRoleWithServer as jest.MockedFunction<
    typeof syncWalletRoleWithServer
  >;
  ...
  describe("validateJwt", () => {
    ...
    describe("Token validation flow", () => {
      ...
      it("should successfully refresh token and update storage", async () => {
        ...
        mockedJwtDecode
          .mockReturnValueOnce(mockPayload) // First call for initial validation
    ...
    describe("Role synchronization tests", () => {
      ...
      it("should not log when local role matches server role", async () => {
        ...
        expect(logErrorSecurely).not.toHaveBeenCalledWith(
          "JWT_ROLE_UPDATE",
          expect.anything()
        );
      });
