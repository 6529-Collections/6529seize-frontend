...
describe("saving Curation drop order", () => {
  ...
  it.each(["first", "last"] as const)(
    "moves to %s without fetching every post",
    async (placement) => {
      fetchMock
        .mockResolvedValueOnce([membership(450)])
        .mockResolvedValueOnce({ data: [{ id: "first-post" }] })
        .mockResolvedValueOnce([membership(500)]);
      await expect(
        moveCurationDrop({
          dropId: "source",
          curationId: "curation",
      ...
      ).resolves.toEqual({ position: placement === "first" ? 1 : 500 });
      expect(fetchMock).toHaveBeenCalledWith({
        endpoint: "v2/waves/wave/curations/curation/drops",
      ...
      expect(postMock).toHaveBeenCalledWith(
        expect.objectContaining({
          body: {
            ...
            priority_order: placement === "first" ? 500 : 1,
          },
        })
      );
    }
  ...
  it("rejects a first/last move when the Curation becomes empty", async () => {
    fetchMock
      .mockResolvedValueOnce([membership(1)])
    ...
    ).rejects.toBeInstanceOf(CurationOrderChangedError);
    expect(postMock).not.toHaveBeenCalled();
  });
});
