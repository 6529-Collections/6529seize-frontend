import {
  ...
  distributionPlanApiDelete,
} from "@/services/distribution-plan-api";

jest.mock("@/services/distribution-plan.utils", () => ({
  makeErrorToast: jest.fn(),
}));
...
const { makeErrorToast } = jest.requireMock(
  "../../services/distribution-plan.utils"
);
const { getAuthJwt, removeAuthJwt } = jest.requireMock(
