...
it("shows the bound price and delivery while keeping a single-copy form compact", () => {
  ...
  const buy = screen.getByRole("button", { name: /Collect 0\.1 ETH/ });
  expect(buy.compareDocumentPosition(screen.getByText("collector.eth"))).toBe(
...
it("requires explicit acknowledgement for a fren's exact address", () => {
  ...
  expect(
    screen.getByRole("button", { name: /Collect 0\.1 ETH/ })
  ).toBeDisabled();
