...
const mockUseSeizeConnectContext = require('../../../components/auth/SeizeConnectContext').useSeizeConnectContext as jest.Mock;
const mockUseSeizeSettings = require('../../../contexts/SeizeSettingsContext').useSeizeSettings as jest.Mock;
...
const mockCommonApiFetch = require('../../../services/api/common-api').commonApiFetch as jest.Mock;
...
describe('RememeAddPage', () => {
  ...
  it('renders page with logo and basic content', () => {
    ...
    expect(screen.getByText(/Please use this page to only add ReMemes/)).toBeInTheDocument();
  ...
  it('shows TDH requirement check for non-deployer', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/You need 6,942 TDH to add this Rememe/)).toBeInTheDocument();
    });
  });
  ...
  it('shows deployer privilege when user is contract deployer', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Rememe can be added (Rememe Contract Deployer)')).toBeInTheDocument();
    });
  });
