...
jest.mock('ethers', () => ({
  ethers: { Wallet: { createRandom: () => ({ address: '0x1', mnemonic: { phrase: 'p' }, privateKey: 'k' }) } }
}));
...
describe('AppWalletsContext methods', () => {
  it('creates and deletes wallet', async () => {
    ...
    const createResult = await act(async () => await result.current.createAppWallet('n','p'));
    
    // Test wallet deletion 
    const deleteResult = await act(async () => await result.current.deleteAppWallet('0x1'));
