...
describe("AppKitAdapterManager", () => {
  ...
  describe("Security Tests - Critical", () => {
    ...
    describe("Sensitive Data Exposure Prevention", () => {
      ...
      it("should throw WalletSecurityError for invalid private key type", () => {
        ...
        expect(() =>
          manager.createAdapter([walletWithInvalidPrivateKeyType])
        ).toThrow(WalletValidationError);
        expect(() =>
          manager.createAdapter([walletWithInvalidPrivateKeyType])
      ...
      it("should throw WalletSecurityError for invalid mnemonic type", () => {
        ...
        expect(() =>
          manager.createAdapter([walletWithInvalidMnemonicType])
        ).toThrow(WalletValidationError);
        expect(() =>
          manager.createAdapter([walletWithInvalidMnemonicType])
      ...
      it("should safely handle wallets with both valid private key and mnemonic", () => {
        expect(() =>
          manager.createAdapter([mockWalletWithSensitiveData])
        ).not.toThrow();
      });
    });
    ...
    describe("Error Code Security", () => {
      it("should use indexed error codes for tracking without exposing sensitive info", () => {
        expect(() => manager.createAdapter(null as any)).toThrow(
          "ADAPTER_007: appWallets must be an array"
        );
  ...
  describe("Constructor Validation", () => {
    ...
    it("should throw indexed AdapterError when requestPassword is not a function", () => {
      expect(() => new AppKitAdapterManager("not-a-function" as any)).toThrow(
        AdapterError
      );
      expect(() => new AppKitAdapterManager("not-a-function" as any)).toThrow(
        "ADAPTER_006: requestPassword must be a function"
      );
    });
  ...
  describe("Wallet Field Validation", () => {
    ...
    it("should throw WalletValidationError when wallet name is missing", () => {
      ...
      expect(() => manager.createAdapter([invalidWallet])).toThrow(
        "Wallet missing required name field"
      );
    });
  ...
  describe("Input Validation Tests", () => {
    ...
    describe("createAdapter", () => {
      it("should throw indexed AdapterError when appWallets is not an array", () => {
        ...
        expect(() => manager.createAdapter("not-array" as any)).toThrow(
          AdapterError
        );
        expect(() => manager.createAdapter(undefined as any)).toThrow(
          AdapterError
        );
      });
    ...
    describe("shouldRecreateAdapter", () => {
      it("should throw indexed AdapterError when newWallets is not an array", () => {
        expect(() => manager.shouldRecreateAdapter(null as any)).toThrow(
          AdapterError
        );
        expect(() => manager.shouldRecreateAdapter(null as any)).toThrow(
          "ADAPTER_009: newWallets must be an array"
        );
      });
      ...
      it("should throw indexed AdapterError when currentWallets contains invalid wallet", () => {
        ...
        expect(() => manager.shouldRecreateAdapter([mockWallet])).toThrow(
          AdapterError
        );
        expect(() => manager.shouldRecreateAdapter([mockWallet])).toThrow(
    ...
    describe("createAdapterWithCache", () => {
      it("should throw indexed AdapterError when appWallets is not an array", () => {
        expect(() => manager.createAdapterWithCache(null as any)).toThrow(
          AdapterError
        );
        expect(() => manager.createAdapterWithCache(null as any)).toThrow(
          "ADAPTER_012: appWallets must be an array"
        );
      });
      ...
      it("should throw indexed AdapterCacheError when cached adapter is null", () => {
        ...
        expect(() => manager.createAdapterWithCache([mockWallet])).toThrow(
    ...
    describe("getConnectionState", () => {
      it("should throw indexed AdapterError when walletAddress is not provided", () => {
        ...
        expect(() => manager.getConnectionState("")).toThrow(
          "ADAPTER_015: walletAddress is required"
        );

        expect(() => manager.getConnectionState(null as any)).toThrow(
          AdapterError
        );
        expect(() => manager.getConnectionState(undefined as any)).toThrow(
          AdapterError
        );
      });

      it("should throw indexed AdapterError when walletAddress is not a string", () => {
        expect(() => manager.getConnectionState(123 as any)).toThrow(
          AdapterError
        );
        expect(() => manager.getConnectionState(123 as any)).toThrow(
          "ADAPTER_016: walletAddress must be a string"
        );
      });
    });
    ...
    describe("setConnectionState", () => {
      it("should throw indexed AdapterError when walletAddress is not provided", () => {
        expect(() => manager.setConnectionState("", "connected")).toThrow(
          AdapterError
        );
        expect(() => manager.setConnectionState("", "connected")).toThrow(
          "ADAPTER_017: walletAddress is required"
        );
      });

      it("should throw indexed AdapterError when walletAddress is not a string", () => {
        expect(() =>
          manager.setConnectionState(123 as any, "connected")
      ...
      it("should throw indexed AdapterError when state is not provided", () => {
        expect(() => manager.setConnectionState("0x123", "" as any)).toThrow(
          AdapterError
        );
        expect(() => manager.setConnectionState("0x123", "" as any)).toThrow(
          "ADAPTER_019: state is required"
        );
      });

      it("should throw indexed AdapterError when state is invalid", () => {
        expect(() =>
          manager.setConnectionState("0x123", "invalid" as any)
        ).toThrow(AdapterError);
        expect(() =>
          manager.setConnectionState("0x123", "invalid" as any)
        ).toThrow(
          "ADAPTER_020: Invalid state: invalid. Must be 'connecting', 'connected', or 'disconnected'"
        );
      });
    });
  });
  ...
  describe("Cache Cleanup Error Handling", () => {
    ...
    describe("performAdapterCleanup", () => {
      it("should throw indexed AdapterCleanupError when adapter is null", () => {
        expect(() =>
          manager["performAdapterCleanup"](null as any, "test-key")
      ...
      it("should throw indexed AdapterCleanupError when cacheKey is empty", () => {
        ...
        expect(() => manager["performAdapterCleanup"](mockAdapter, "")).toThrow(
      ...
      it("should throw indexed AdapterCleanupError when trying to cleanup current adapter", () => {
        ...
        expect(() =>
          manager["performAdapterCleanup"](adapter, "test-key")
      ...
      it("should cleanup non-current adapter successfully", () => {
        ...
        expect(() =>
          manager["performAdapterCleanup"](adapter1, "test-key")
        ).not.toThrow();
      });
    });
    ...
    describe("cleanup method", () => {
      it("should handle cleanup errors and throw indexed AdapterCleanupError", () => {
        ...
        jest
          .spyOn(manager as any, "performAdapterCleanup")
          .mockImplementation(() => {
            throw new Error("Mock cleanup error");
          });

        expect(() => manager.cleanup()).toThrow(
  ...
  describe("Connection State Validation", () => {
    ...
    it("should return disconnected for unknown wallet address", () => {
      expect(manager.getConnectionState("unknown-address")).toBe(
        "disconnected"
      );
    });
  ...
  describe("Cache Key Generation", () => {
    ...
    it("should throw indexed AdapterError when wallets is not an array", () => {
      ...
      expect(() => manager["getCacheKey"](null as any)).toThrow(
  ...
  describe("Type Safety Tests", () => {
    ...
    it("should maintain type safety for connection states", () => {
      ...
      const state: "connecting" | "connected" | "disconnected" =
        manager.getConnectionState("0x123");
  ...
  describe("Functional Behavior Tests", () => {
    ...
    it("should detect when adapter recreation is needed", () => {
      ...
      expect(manager.shouldRecreateAdapter([mockWallet, mockWallet2])).toBe(
        true
      );
    ...
    it("should detect chain changes when checking adapter recreation", () => {
      ...
      expect(manager.shouldRecreateAdapter([mockWallet], [mainnet])).toBe(
        false
      );
    ...
    it("should not reuse cached adapter across different chains", () => {
      const adapter1 = manager.createAdapterWithCache([mockWallet], false, [
        mainnet,
      ]);
      const adapter2 = manager.createAdapterWithCache([mockWallet], false, [
        sepolia,
      ]);
  ...
  describe("Error Propagation", () => {
    ...
    it("should propagate WagmiAdapter constructor errors", () => {
      ...
      expect(() => manager.createAdapter([mockWallet])).toThrow(
        "WagmiAdapter creation failed"
      );
    });
