...
jest.mock(
  "@/components/user/proxy/proxy/list/ProxyActionRowStatus",
  () => (props: any) => (
...
jest.mock(
  "@/components/user/proxy/proxy/action/utils/credit/ProfileProxyCredit",
  () => (props: any) => (
    <button data-testid="credit" onClick={props.onCreditEdit}>
      credit
    </button>
  )
);
...
jest.mock(
  "@/components/user/proxy/proxy/action/utils/credit/ProfileProxyCreditSpent",
  () => () => <div data-testid="spent">spent</div>
);

jest.mock(
  "@/components/user/proxy/proxy/action/utils/time/ProfileProxyEndTime",
  () => (props: any) => (
    <button data-testid="end" onClick={props.onEndTimeEdit}>
      end
    </button>
  )
);
...
jest.mock(
  "@/components/user/proxy/proxy/action/ProxyActionAcceptanceButton",
...
test("renders labels and statuses", () => {
  ...
  expect(
    screen.getByText(
      PROFILE_PROXY_ACTION_LABELS[ApiProfileProxyActionType.AllocateRep]
    )
  ).toBeInTheDocument();
