...
jest.mock("services/distribution-plan-api", () => ({
  distributionPlanApiFetch: jest.fn(async () => ({
    success: true,
    data: [{ wallet: "0x1", amount: 1, phaseId: "p1", phaseComponentId: "c1" }],
  })),
}));
...
jest.mock(
  "components/distribution-plan-tool/common/RoundedJsonIconButton",
  () => (props: any) => (
    <button data-testid="json-btn" onClick={props.onClick} />
  )
);
jest.mock(
  "components/distribution-plan-tool/common/RoundedCsvIconButton",
  () => (props: any) => <button data-testid="csv-btn" onClick={props.onClick} />
);
jest.mock(
  "components/distribution-plan-tool/common/RoundedManifoldIconButton",
  () => (props: any) => (
    <button data-testid="manifold-btn" onClick={props.onClick} />
  )
);
jest.mock(
  "components/distribution-plan-tool/review-distribution-plan/table/ReviewDistributionPlanTableSubscription",
  () => ({ SubscriptionLinks: () => <div /> })
);
...
function setup() {
  ...
  return render(
    <DistributionPlanToolContext.Provider value={{ distributionPlan } as any}>
      <table>
        <tbody>
          <ReviewDistributionPlanTableRow item={item} rows={rows} />
        </tbody>
      </table>
...
describe("ReviewDistributionPlanTableRow", () => {
  ...
  it("aggregates values when downloading manifold csv", async () => {
    (distributionPlanApiFetch as jest.Mock).mockResolvedValueOnce({
      success: true,
      data: [
