...
jest.mock(
  "@/components/distribution-plan-tool/plans/DistributionPlanToolPlansLoading",
  () => () => <div data-testid="loading" />
);
jest.mock(
  "@/components/distribution-plan-tool/plans/DistributionPlanToolPlansNoPlans",
  () => () => <div data-testid="no-plans" />
);

// Mock table component to inspect props and trigger onDeleted
const mockOnDeleted = jest.fn();
jest.mock(
  "@/components/distribution-plan-tool/plans/DistributionPlanToolPlansTable",
  () => (props: any) =>
    (
      <div data-testid="table">
        <span data-testid="count">{props.plans.length}</span>
        <button
          onClick={() => props.onDeleted(props.plans[0].id)}
...
describe("DistributionPlanToolPlans", () => {
  ...
  it("renders plans table and handles deletion", async () => {
    mockedFetch.mockResolvedValue({
      success: true,
      data: [{ id: "1", name: "plan", description: "", createdAt: 0 }],
    });
    render(<DistributionPlanToolPlans />);
