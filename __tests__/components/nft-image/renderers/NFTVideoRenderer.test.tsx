...
jest.mock("@/components/nft-image/NFTImageBalance", () => {
  return function MockNFTImageBalance({ contract, tokenId, height }: any) {
    // Simple deterministic mock: 1 => seized x1, 0 => UNSEIZED, else => loading ...
...
describe("NFTVideoRenderer", () => {
  ...
  describe("Props Propagation", () => {
    ...
    it("propagates balance-related props to NFTImageBalance", () => {
      ...
      expect(seizedText).toHaveTextContent("SEIZED x1");
    });
    ...
    it("handles optional props correctly", () => {
      ...
      const { container } = render(
        <NFTVideoRenderer {...propsWithoutOptionals} />
      );
      const video = container.querySelector("video");
      expect(video).toBeInTheDocument();
  ...
  describe("Video Source Priority Logic", () => {
    it("prioritizes compressed_animation over animation when showOriginal is false", () => {
      ...
      expect(video).toHaveAttribute(
        "src",
        "https://priority.com/compressed.mp4"
      );
    });
    ...
    it("handles NFTLite type correctly", () => {
      ...
      expect(video).toHaveAttribute(
        "src",
        "https://nftlite.com/compressed.mp4"
      );
    });
  });
  ...
  describe("Video Source Logic Edge Cases", () => {
    it("handles metadata check correctly for BaseNFT vs NFTLite", () => {
      ...
      const propsWithMetadata = createDefaultProps({
        nft: nftWithMetadata,
        showOriginal: false,
      });
      const { container: container1 } = render(
        <NFTVideoRenderer {...propsWithMetadata} />
      );
      const video1 = container1.querySelector("video");
      expect(video1).toHaveAttribute(
        "src",
      ...
      const propsWithoutMetadata = createDefaultProps({
        nft: nftWithoutMetadata as any,
        showOriginal: false,
      });
      const { container: container2 } = render(
        <NFTVideoRenderer {...propsWithoutMetadata} />
      );
    ...
    it("prioritizes showOriginal flag over compressed_animation", () => {
      ...
      const { container: container1 } = render(
        <NFTVideoRenderer {...propsOriginal} />
      );
      const video1 = container1.querySelector("video");
      ...
      const { container: container2 } = render(
        <NFTVideoRenderer {...propsCompressed} />
      );
      const video2 = container2.querySelector("video");
      expect(video2).toHaveAttribute(
        "src",
        "https://example.com/compressed.mp4"
      );
    });
