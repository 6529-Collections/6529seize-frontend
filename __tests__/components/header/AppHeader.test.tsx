...
jest.mock('../../../components/header/AppSidebar', () => ({ __esModule: true, default: (props: any) => <div data-testid="sidebar" {...props} /> }));
...
jest.mock('../../../contexts/NavigationHistoryContext', () => ({ useNavigationHistoryContext: jest.fn() }));
jest.mock('../../../components/navigation/BackButton', () => ({ __esModule: true, default: () => <div data-testid="back" /> }));
jest.mock('../../../components/utils/Spinner', () => ({ __esModule: true, default: () => <div data-testid="spinner" /> }));
jest.mock('../../../components/header/HeaderActionButtons', () => ({ __esModule: true, default: () => <div data-testid="actions" /> }));
...
const { useNavigationHistoryContext } = require('../../../contexts/NavigationHistoryContext');

function setup(opts: any) {
  (useSeizeConnectContext as jest.Mock).mockReturnValue({ address: opts.address });
  ...
  (useViewContext as jest.Mock).mockReturnValue({ activeView: opts.activeView ?? null });
  (useNavigationHistoryContext as jest.Mock).mockReturnValue({ canGoBack: opts.canGoBack ?? false });
  (useWaveById as jest.Mock).mockReturnValue({ wave: opts.wave, isLoading: false, isFetching: false });
  (useRouter as jest.Mock).mockReturnValue({
    asPath: opts.asPath,
    pathname: opts.pathname ?? opts.asPath,
    query: opts.query ?? {},
  });
...
describe('AppHeader', () => {
  ...
  it('shows menu icon when not connected', () => {
    ...
    expect(screen.getByRole('button', { name: 'Open menu' }).querySelector('svg')).toBeInTheDocument();
  });
  ...
  it('shows profile image when connected without back button', () => {
    setup({ address: '0xabc', profile: { pfp: 'pfp.png' }, asPath: '/home', activeView: 'messages' });
