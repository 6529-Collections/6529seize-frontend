...
jest.mock("@/components/nothingHereYet/NothingHereYetSummer", () => ({
  __esModule: true,
  default: () => <div data-testid="NothingHereYet" />,
}));
jest.mock("@/components/nftAttributes/NFTAttributes", () => ({
  __esModule: true,
  default: () => <div data-testid="NFTAttributes" />,
}));
jest.mock("@/components/nftAttributes/NftStats", () => ({
  __esModule: true,
  NftPageStats: () => <tr data-testid="NftStats" />,
}));
jest.mock("@/components/rememes/RememePage", () => ({
  printMemeReferences: jest.fn(() => <div data-testid="MemeRefs" />),
}));
jest.mock("@/components/nft-marketplace-links/NFTMarketplaceLinks", () => ({
  __esModule: true,
  default: () => <div data-testid="MarketLinks" />,
}));
jest.mock("next/link", () => ({
  __esModule: true,
...
beforeEach(() => {
  ...
  useSearch.mockReturnValue(new URLSearchParams("id=1"));
});
...
describe("MemeLabPage", () => {
  ...
  it("shows transaction history on your cards tab", async () => {
    (useRouter as jest.Mock).mockReturnValue({ replace: jest.fn() });
    useSearch.mockReturnValue(new URLSearchParams('id=1&focus=your-cards'));
    mockNftCalls(2);
    render(<LabPage wallets={["0xabc"]} />);
    await waitFor(() =>
      expect(fetchUrl).toHaveBeenCalledWith(
