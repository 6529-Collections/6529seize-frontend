...
jest.mock(
  "@/components/user/utils/icons/CommonTableSortIcon",
  () => (props: any) => <div data-testid="sort">{props.direction}</div>
);
...
test("handles copy feedback", () => {
  ...
  const Child = ({ onCopy }: { onCopy: () => void }) => (
    <button data-testid="child" onClick={onCopy} />
  );
  render(
    <CommonDropdownItem
      item={item}
