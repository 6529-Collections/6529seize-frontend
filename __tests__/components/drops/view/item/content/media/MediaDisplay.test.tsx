...
describe("MediaDisplay", () => {
  ...
  it("renders attachment card for csv extension when mime type is missing", () => {
    render(<MediaDisplay media_mime_type="" media_url="file.csv?download=1" />);
    const node = screen.getByTestId("attachment");
    expect(node).toHaveAttribute("data-src", "file.csv?download=1");
    expect(node).toHaveAttribute("data-mime", "");
  });
