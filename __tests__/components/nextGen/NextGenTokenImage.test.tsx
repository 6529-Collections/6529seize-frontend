...
test("renders image without link when hide_link is true", () => {
  const { container } = render(<NextGenTokenImage token={token} hide_link show_original />);
  const image = screen.getByAltText("token");
  expect(image).toBeInTheDocument();
  expect(image).toHaveAttribute("src", "image.png");
...
test("renders iframe when animation shown and link hidden", () => {
  ...
  render(<NextGenTokenImage token={tokenWithAnimation} show_animation hide_link />);
...
test("renders listing info when show_listing is true and price > 0", () => {
  const tokenWithPrice = { ...token, price: 1, opensea_price: 1, opensea_royalty: 500 };
...
test("does not render royalty image when prices do not match", () => {
  const tokenWithMismatchedPrice = { ...token, price: 1, opensea_price: 2, opensea_royalty: 500 };
...
test("renders Magic Eden royalty image when me_price matches token price", () => {
  const tokenWithMEPrice = { ...token, price: 1.5, me_price: 1.5, me_royalty: 750 };
...
test("does not render Magic Eden royalty image when me_price does not match token price", () => {
  const tokenWithMismatchedMEPrice = { ...token, price: 1.5, me_price: 2.0, me_royalty: 750 };
...
test("renders image when show_animation is false", () => {
  const tokenWithAnimation = { ...token, animation_url: "anim.html" };
  render(<NextGenTokenImage token={tokenWithAnimation} show_animation={false} hide_link />);
...
test("renders image when show_animation is true but no animation_url", () => {
  ...
  render(<NextGenTokenImage token={tokenWithoutAnimation} show_animation hide_link />);
...
test("renders with proper container structure for regular mode", () => {
  ...
  expect(imageContainer).toHaveClass("d-flex", "flex-column", "align-items-center", "justify-content-center");
});
...
test("renders different marketplace prices in owner info tooltip", () => {
  ...
  render(<NextGenTokenImage token={tokenWithMarketplacePrices} show_owner_info />);
