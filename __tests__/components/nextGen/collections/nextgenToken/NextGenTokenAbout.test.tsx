...
jest.mock("@fortawesome/react-fontawesome", () => ({
  FontAwesomeIcon: (p: any) => <svg data-testid={p.icon?.iconName || "icon"} />,
}));
...
jest.mock("@/services/api/common-api", () => ({
  commonApiFetch: jest.fn(() =>
    Promise.resolve({ data: [{ boosted_tdh: 5 }] })
  ),
}));
...
jest.mock("@/hooks/useIdentity", () => ({
  useIdentity: () => ({
    profile: { level: 1, cic: "REP", handle: "me", tdh: 2 },
  }),
}));
jest.mock("@/components/cookies/CookieConsentContext", () => ({
  useCookieConsent: () => ({ country: "US" }),
}));
jest.mock("@/components/user/utils/icons/EthereumIcon", () => () => (
  <span data-testid="eth" />
));
