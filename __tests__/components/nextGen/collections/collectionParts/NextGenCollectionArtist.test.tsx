...
beforeEach(() => {
  (useIdentity as jest.Mock).mockReturnValue({
    profile: { pfp: "pfp.jpg", handle: "alice" },
  });
...
describe("NextGenCollectionArtist", () => {
  it("renders profile info and bio", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText("bio text")).toBeInTheDocument()
    );

    expect(screen.getByRole("link", { name: "Another" })).toHaveAttribute(
      "href",
      `/nextgen/collection/${formatNameForUrl("Another")}`
    );
