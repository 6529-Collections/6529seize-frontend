...
jest.mock(
  "@/components/nextGen/collections/collectionParts/hooks/SlideshowHeader",
  () => {
    return function MockSlideshowHeader({
      collectionName,
    }: {
      collectionName: string;
    }) {
      return (
        <div data-testid="slideshow-header">Header for {collectionName}</div>
      );
    };
  }
);
...
jest.mock(
  "@/components/nextGen/collections/collectionParts/hooks/TokenSlideshow",
  () => {
    return function MockTokenSlideshow({
      collectionId,
      initialTokens,
    }: {
      collectionId: number;
      initialTokens?: any[] | undefined;
    }) {
      return (
        <div data-testid="token-slideshow">
          Collection ID: {collectionId}
          {initialTokens && (
            <div data-testid="initial-tokens">
              Initial tokens: {initialTokens.length}
            </div>
          )}
        </div>
      );
...
describe("NextGenCollectionSlideshow", () => {
  it("renders slideshow container with Tailwind classes", () => {
    const { container } = render(
      <NextGenCollectionSlideshow collection={mockCollection} />
    );

    const slideshowContainer = container.firstElementChild;
    expect(slideshowContainer).toHaveClass(
      "tw-w-full",
      "tw-rounded-xl",
      "tw-border-white/10",
      "tw-bg-iron-900"
  ...
  it("passes initial tokens to slideshow when provided", () => {
    ...
    const slideshow = screen.getByTestId("token-slideshow");
    expect(slideshow).toBeInTheDocument();
    expect(screen.getByTestId("initial-tokens")).toHaveTextContent(
      "Initial tokens: 3"
    );
  });
