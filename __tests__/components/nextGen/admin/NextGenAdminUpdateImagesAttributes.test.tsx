...
const { useCoreContractWrite } = require('../../../../components/nextGen/nextgen_helpers');
jest.mock('../../../../components/nextGen/NextGenContractWriteStatus', () => {
  return function MockNextGenContractWriteStatus({ isLoading, hash, error }: any) {
    return (
...
describe('NextGenAdminUpdateImagesAttributes', () => {
  ...
  const defaultContractWrite = {
    ...
    data: undefined,
    error: null,
    params: { address: '0x123' as `0x${string}`, abi: [], chainId: 1, functionName: 'test' }
  };
  ...
  describe('Component Rendering', () => {
    it('renders all form fields with correct labels', () => {
      ...
      expect(screen.getByText('Update Images and Attributes')).toBeInTheDocument();
  ...
  describe('Form Validation', () => {
    it('shows validation errors when fields are empty', async () => {
      ...
      expect(screen.getByText('At least one token ID is required')).toBeInTheDocument();
      expect(screen.getByText('At least one image is required')).toBeInTheDocument();
      expect(screen.getByText('At least one attribute is required')).toBeInTheDocument();
    });
    ...
    it('shows validation error when array lengths do not match', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Number of entries for token IDs, images and attributes must all be the same')).toBeInTheDocument();
      });
    });
