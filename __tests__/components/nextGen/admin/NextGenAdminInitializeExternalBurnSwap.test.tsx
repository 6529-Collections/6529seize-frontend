...
const signMessageState: any = { signMessage: jest.fn(), reset: jest.fn(), isError: false, isSuccess: false, data: undefined };
const contractWriteState: any = { writeContract: jest.fn(), reset: jest.fn(), params: {}, isLoading: false, isSuccess: false, isError: false };
...
test('calls signMessage when form valid', async () => {
  ...
  fireEvent.click(screen.getByText('Submit'));
  expect(signMessageState.signMessage).toHaveBeenCalledWith({ message: 'uuid' });
});
