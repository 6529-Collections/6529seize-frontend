...
describe("useSharedState", () => {
  ...
  it("includes custom block parameters when set", () => {
    ...
    expect(url).toBe(
      "https://api.test.6529.io/api/royalties/collection/memelab?&from_block=10&to_block=20"
    );
  });
  ...
  it("includes artist filter when selectedArtist is set", () => {
    ...
    expect(url).toBe(
      "https://api.test.6529.io/api/gas/collection/memes?&from_date=2024-01-01&to_date=2024-01-02&artist=TestArtist"
    );
  });
  ...
  it("handles MEMELAB collection focus correctly", () => {
    ...
    const url = result.current.getUrl("royalties");
    expect(url).toBe(
      "https://api.test.6529.io/api/royalties/collection/memelab?&primary=true"
    );
  });
