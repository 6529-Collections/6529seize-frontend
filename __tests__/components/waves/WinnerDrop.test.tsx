...
jest.mock("@/components/memes/drops/MemeWinnerDrop", () => (props: any) => (
  <div data-testid="meme" />
));
jest.mock(
  "@/components/waves/drops/winner/DefaultWinnerDrop",
  () => (props: any) => (
    <div data-testid="default" data-presentation={props.contentPresentation} />
  )
);
...
describe("WinnerDrop", () => {
  ...
  it("renders meme winner when wave is memes", () => {
    const { getByTestId } = render(
      <WinnerDrop {...baseProps} drop={{ wave: { id: "meme" } }} />
    );
  ...
  it("renders default winner otherwise", () => {
    ...
    expect(getByTestId("default")).toHaveAttribute(
      "data-presentation",
      "proposalCard"
    );
  });
