...
jest.mock(
  "@/components/waves/leaderboard/sidebar/WaveLeaderboardRightSidebarActivityLog",
  () => ({
    WaveLeaderboardRightSidebarActivityLog: (props: any) => (
      <div data-testid="log">{props.log.id}</div>
    ),
  })
);
...
describe("WaveLeaderboardRightSidebarActivityLogs", () => {
  ...
  it("shows placeholder when no logs", () => {
    ...
    render(<WaveLeaderboardRightSidebarActivityLogs wave={wave} />);
    expect(screen.getByText("No voting activity yet")).toBeInTheDocument();
  });
