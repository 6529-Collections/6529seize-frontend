...
jest.mock("framer-motion", () => {
  const MotionDivMock = React.forwardRef<HTMLDivElement, any>(
    (
      {
        children,
        className,
...
jest.mock("@/components/waves/memes/file-upload/components/FilePreview", () => {
  const FilePreviewMock = React.forwardRef<HTMLDivElement, any>(
    (props, ref) => (
      <div ref={ref} data-testid="preview">
        {props["url"] || "no-url"}
        {props["file"] && (
          <span data-testid="file-info">{props["file"].name}</span>
        )}
      </div>
    )
  );
...
jest.mock(
  "@/components/waves/memes/file-upload/components/BrowserWarning",
  () => {
    const BrowserWarningMock = React.forwardRef<HTMLDivElement, any>(
      ({ reason }, ref) => {
        if (!reason) {
          throw new Error("BrowserWarning requires reason prop");
        }
        return (
          <div ref={ref} data-testid="warning">
            {reason}
          </div>
        );
      }
...
jest.mock(
  "@/components/waves/memes/file-upload/hooks/useDragAndDrop",
  () => () => ({
...
jest.mock(
  "@/components/waves/memes/file-upload/hooks/useAccessibility",
  () => () => ({
...
jest.mock(
  "@/components/waves/memes/file-upload/utils/browserDetection",
  () => ({
    isBrowserSupported: jest.fn(() => ({
      supported: false,
      reason: "Browser compatibility issue detected",
    })),
...
const mockBrowserDetection = jest.mocked(
  require("@/components/waves/memes/file-upload/utils/browserDetection")
);
...
describe("MemesArtSubmissionFile", () => {
  ...
  const VALID_ARWEAVE_SUBDOMAIN =
    "ifx4blsbsfs22lskmlsb6zsazvhxcoibl4nkk3checvtipo6hkhq";
  ...
  beforeEach(() => {
    ...
    mockBrowserDetection.isBrowserSupported.mockReturnValue({
      supported: false,
      reason: "Browser compatibility issue detected",
    });
  ...
  describe("Upload State Management", () => {
    ...
    it("applies minimum-height classes to upload panel for mobile safety", () => {
      ...
      const uploadPanel = screen.getByTestId("artwork-upload-area");
      expect(uploadPanel).toHaveClass("tw-min-h-[280px]");
      expect(uploadPanel).toHaveClass("sm:tw-min-h-[340px]");
    });

    it("applies minimum-height classes to interactive panel for mobile safety", () => {
      const { container } = render(
        <AuthContext.Provider value={{ setToast: mockSetToast } as any}>
          <MemesArtSubmissionFile {...baseProps} mediaSource="url" />
        </AuthContext.Provider>
      );

      const interactivePanel = container.querySelector(
        "#memes-art-submission-interactive-panel"
      ) as HTMLElement | null;
      expect(interactivePanel).toBeInTheDocument();
      if (!interactivePanel) {
        throw new Error("Interactive panel should be rendered");
      }
      expect(interactivePanel).toHaveClass("tw-min-h-[280px]");
      expect(interactivePanel).toHaveClass("sm:tw-min-h-[340px]");
    });
  });
  ...
  describe("URL Management", () => {
    it("creates blob URL when new file is selected", () => {
      ...
      render(
        <AuthContext.Provider value={{ setToast: mockSetToast } as any}>
          <MemesArtSubmissionFile {...baseProps} artworkUrl="original-url" />
        </AuthContext.Provider>
      );
  ...
  describe("Interactive media preview security", () => {
    ...
    it("blocks previews from unapproved domains", () => {
      ...
      expect(
        screen.queryByTitle("Interactive artwork preview")
    ...
    it("shows validation pending message before iframe renders", () => {
      ...
      expect(screen.getByText("Validating preview...")).toBeInTheDocument();
      expect(
        screen.queryByTitle("Interactive artwork preview")
      ).not.toBeInTheDocument();
    });
