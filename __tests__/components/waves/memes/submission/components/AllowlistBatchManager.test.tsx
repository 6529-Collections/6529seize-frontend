...
describe("AllowlistBatchManager", () => {
  ...
  it("renders with no batches and allows adding one", async () => {
    ...
    expect(onBatchesChange).toHaveBeenCalledWith([{ contract: "", token_ids_raw: "" }]);
  });

  it("renders existing batches", () => {
    const batches = [
      { contract: "0x1", token_ids_raw: "1-10" },
      { contract: "0x2", token_ids_raw: "20,21" },
    ];
  ...
  it("calls onBatchesChange when a batch is updated", async () => {
    const user = userEvent.setup();
    const onBatchesChange = jest.fn();
    const batches = [{ contract: "0x1", token_ids_raw: "1" }];
    ...
    expect(onBatchesChange).toHaveBeenCalledWith([{ contract: "0x12", token_ids_raw: "1" }]);
  });
  ...
  it("allows removing a batch", async () => {
    ...
    const onBatchesChange = jest.fn();
    const batches = [
      { contract: "0x1", token_ids_raw: "1" },
      { contract: "0x2", token_ids_raw: "2" },
    ];
    render(<AllowlistBatchManager batches={batches} onBatchesChange={onBatchesChange} />);
    ...
    expect(onBatchesChange).toHaveBeenCalledWith([{ contract: "0x2", token_ids_raw: "2" }]);
  });
});
