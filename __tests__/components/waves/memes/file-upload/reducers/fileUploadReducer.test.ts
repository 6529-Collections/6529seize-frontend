...
} from "@/components/waves/memes/file-upload/reducers/fileUploadReducer";
...
describe("fileUploaderReducer", () => {
  it("sets visual state", () => {
    const state = fileUploaderReducer(initialFileUploaderState, {
  ...
  it("handles processing success", () => {
    const action = {
      ...
      payload: { objectUrl: "url", file: sampleFile },
    } as any;
  ...
  it("increments attempts on start processing and handles error", () => {
    const state1 = fileUploaderReducer(initialFileUploaderState, {
      type: "START_PROCESSING",
      payload: sampleFile,
    } as any);
    expect(state1.processingAttempts).toBe(1);
    const state2 = fileUploaderReducer(state1, {
  ...
  it("handles timeout and compatibility check", () => {
    const state = fileUploaderReducer(initialFileUploaderState, {
      type: "START_COMPATIBILITY_CHECK",
      payload: sampleFile,
    } as any);
    expect(state.isCheckingCompatibility).toBe(true);
    const timeout = fileUploaderReducer(state, {
      type: "PROCESSING_TIMEOUT",
    } as any);
    expect(timeout.error).toMatch("timed out");
    const compat = fileUploaderReducer(state, {
      type: "SET_COMPATIBILITY_RESULT",
      payload: { tested: true, canPlay: true },
    } as any);
    expect(compat.videoCompatibility).toEqual({ tested: true, canPlay: true });
  });

  it("clears retry state immediately for validation errors", () => {
    const processing = fileUploaderReducer(initialFileUploaderState, {
      type: "START_PROCESSING",
      payload: sampleFile,
    });
    const state = fileUploaderReducer(processing, {
      type: "VALIDATION_ERROR",
      payload: "File size exceeds 250 MB limit.",
    });
