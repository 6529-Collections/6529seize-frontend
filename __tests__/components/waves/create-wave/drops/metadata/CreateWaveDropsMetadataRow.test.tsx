...
jest.mock(
  "@/components/waves/create-wave/drops/metadata/CreateWaveDropsMetadataRowType",
  () => (props: any) => (
    <div
      data-testid="type"
      onClick={() => props.onTypeChange(ApiWaveMetadataType.Number)}
    />
  )
);
  ...
  it("calls onItemChange when input changes", async () => {
    ...
    render(
      <CreateWaveDropsMetadataRow
        ...
        index={0}
        errorMessage={IDENTITY_SUBMISSION_RESERVED_METADATA_ERROR}
    ...
    expect(onItemChange).toHaveBeenLastCalledWith(
      expect.objectContaining({ type: ApiWaveMetadataType.Number })
    );
    await user.click(screen.getByRole("button", { name: "Remove item" }));
    expect(onItemRemove).toHaveBeenCalledWith(0);
    expect(
      screen.getByText(IDENTITY_SUBMISSION_RESERVED_METADATA_ERROR)
    ).toBeInTheDocument();
  });
});
