  ...
  () => {
    return {
      WaveSmallLeaderboardItemContent:
        function MockWaveSmallLeaderboardItemContent({
          drop,
          onDropClick,
        }: any) {
          return (
  ...
  () => {
    return {
      WaveSmallLeaderboardItemOutcomes:
        function MockWaveSmallLeaderboardItemOutcomes({ drop, wave }: any) {
    ...
    default: function MockWinnerDropBadge({ rank, decisionTime }: any) {
      return (
        <div data-testid="winner-badge">
          Winner Badge - Rank: {rank}{" "}
          {decisionTime ? `Time: ${decisionTime}` : ""}
        </div>
      );
    },
  };
...
describe("WaveSmallLeaderboardTopThreeDrop", () => {
  ...
  const createMockDrop = (
    overrides: Partial<ExtendedDrop> = {}
  ): ExtendedDrop =>
    ({
      id: "drop-1",
      rank: 1,
  ...
  it("renders author information correctly", () => {
    ...
    expect(
      screen.getByAltText("testuser's profile picture")
    ).toBeInTheDocument(); // PFP
  });
  ...
  it("renders placeholder when author has no pfp", () => {
    ...
    const placeholderDiv = document.querySelector(
      ".tw-size-6.tw-flex-shrink-0.tw-rounded-lg.tw-bg-iron-800"
    );
  ...
  it("has correct hover styling classes", () => {
    ...
    const dropContainer = container.querySelector(
  ...
  it("handles missing winning context gracefully", () => {
    const drop = createMockDrop({
      ...(undefined !== undefined ? { winning_context: undefined } : {}),
    });
    render(
      <WaveSmallLeaderboardTopThreeDrop
        drop={drop}
