...
const mockTopThreeDrop = jest.fn(({ drop, onDropClick }: any) => (
  <div data-testid="top-three-drop" onClick={() => onDropClick(drop)}>
    Top Three Drop - Rank {drop.rank}
  </div>
));

const mockDefaultDrop = jest.fn(({ drop, onDropClick }: any) => (
...
jest.mock(
  "@/components/waves/small-leaderboard/WaveSmallLeaderboardTopThreeDrop",
  () => ({
    WaveSmallLeaderboardTopThreeDrop: (props: any) => mockTopThreeDrop(props),
  })
);

jest.mock(
  "@/components/waves/small-leaderboard/WaveSmallLeaderboardDefaultDrop",
  () => ({
    WaveSmallLeaderboardDefaultDrop: (props: any) => mockDefaultDrop(props),
  })
);
...
describe("DefaultWaveSmallLeaderboardDrop", () => {
  ...
  it("renders top three component for rank 1", () => {
  ...
  it("calls onDropClick when container is clicked", async () => {
    ...
    expect(mockOnDropClick).toHaveBeenCalled();
  });
  ...
  it("handles edge case of rank 0", () => {
    ...
    expect(screen.getByTestId("top-three-drop")).toBeInTheDocument();
    expect(screen.getByText("Top Three Drop - Rank 0")).toBeInTheDocument();
    expect(screen.queryByTestId("default-drop")).not.toBeInTheDocument();
  });
