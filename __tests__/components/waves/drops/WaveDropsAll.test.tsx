...
describe("WaveDropsAll", () => {
  ...
  describe("Content Rendering", () => {
    it("renders drops list when drops are present", () => {
      const mockDrops = [
        createMockDrop({ id: "drop-1" }),
        createMockDrop({ id: "drop-2" }),
      ];
      setupMocks({
        waveMessages: {
          drops: mockDrops,
    ...
    it("passes correct props to DropsList component", () => {
      ...
      const mockActiveDrop: ActiveDropState = {
        drop: createMockDrop({ id: "drop-1" }),
        partId: 1,
        action: "reply" as any,
      };
  ...
  describe("Component Lifecycle", () => {
    ...
    it("uses bottomAnchorRef for robust scroll detection", async () => {
      ...
      expect(
        require("@/hooks/useScrollBehavior").useScrollBehavior
      ).toHaveBeenCalled();
    });
  });
});
