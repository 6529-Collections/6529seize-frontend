...
describe('SingleWaveDropTraits', () => {
  const createMockDrop = (metadata: ApiDropMetadata[] = [], title = 'Test Drop'): ExtendedDrop => ({
    id: 'test-drop-id',
    serial_no: 1,
    author: { id: 'test-author', handle: 'testuser' },
    title,
    parts: [{ part_id: 1, content: 'Test description', media: [], quoted_drop: null }],
    metadata,
    created_at: Date.now(),
    updated_at: Date.now(),
    wave: { id: 'test-wave' },
  ...
  describe('number trait extraction', () => {
    it('should extract and display number traits', () => {
      ...
      const hasAnyNumber = ['85', '67', '92', '78'].some(num => 
        screen.queryByText(num) !== null
      );
  ...
  describe('trait prioritization and ordering', () => {
    it('should display traits in priority order', () => {
      ...
      const artistIndex = labelTexts.findIndex(text => text?.includes('Artist'));
      const weaponIndex = labelTexts.findIndex(text => text?.includes('Weapon'));
  ...
  describe('show more/less functionality', () => {
    ...
    it('should not show "Show all" button when only 2 or fewer traits exist', () => {
      const drop = createMockDrop([
        createMetadata('artist', 'Artist 1'),
      ], ''); // Only one string trait, plus default boolean traits
      drop.parts = [];
  ...
  describe('component structure', () => {
    ...
    it('should render metadata items with correct styling', () => {
      ...
      const { container } = render(<SingleWaveDropTraits drop={drop} />);
      const metadataItems = container.querySelectorAll('.tw-px-2.tw-py-1.tw-rounded-md.tw-bg-iron-800');
  ...
  describe('fallback values', () => {
    it('should use drop title and description when available', () => {
      ...
      drop.parts = [{ part_id: 1, content: 'Drop Description', media: [], quoted_drop: null }];
    ...
    it('should prefer metadata values over drop fallbacks', () => {
      ...
      ], 'Drop Title'); // Have a drop title to test preference
      drop.parts = [{ part_id: 1, content: 'Drop Description', media: [], quoted_drop: null }];
