...
import DropTraits from "../../../../components/waves/drop/DropTraits";
...
describe("DropTraits", () => {
  ...
  describe("rendering with no traits", () => {
    it("should render boolean traits even when no meaningful text traits exist", () => {
      ...
      const { container } = render(<DropTraits drop={drop} />);

      // Component should still render boolean traits like punk6529 No, gradient No
      expect(container.firstChild).not.toBeNull();
      expect(screen.getByText("Punk6529")).toBeInTheDocument();
  ...
  describe("string trait extraction", () => {
    it("should extract and display string traits", () => {
      ...
      render(<DropTraits drop={drop} />);

      expect(screen.getByText("Test Artist")).toBeInTheDocument();
      // Since boolean traits have priority, "Vibrant" may not be in the first 2 items
      // Click "Expand Traits" to see additional traits
      const showAllButton = screen.queryByText("Expand Traits");
      if (showAllButton) {
        fireEvent.click(showAllButton);
        expect(screen.getByText("Vibrant")).toBeInTheDocument();
      } else {
        // If no "Expand Traits" button, "Vibrant" should be visible as second trait
        expect(screen.getByText("Vibrant")).toBeInTheDocument();
      }
    });
  ...
  describe("boolean trait extraction", () => {
    it("should extract and display boolean traits as Yes/No", () => {
      ...
      // Expand Traits traits to ensure we can see all boolean values
      const showAllButton = screen.queryByText("Expand Traits");
      if (showAllButton) {
        fireEvent.click(showAllButton);
      }
    ...
    it("should handle 3D trait mapping to threeD", () => {
      ...
      // Expand Traits traits to see all boolean values
      const showAllButton = screen.queryByText("Expand Traits");
      if (showAllButton) {
        fireEvent.click(showAllButton);
      }
      ...
      const threeDElement = screen.queryByText("Three D");
      if (threeDElement) {
        expect(screen.getByText("Three D")).toBeInTheDocument();
  ...
  describe("number trait extraction", () => {
    it("should extract and display number traits", () => {
      ...
      // Expand Traits traits to see number traits
      const showAllButton = screen.queryByText("Expand Traits");
      if (showAllButton) {
        fireEvent.click(showAllButton);
      }
  ...
  describe("trait prioritization and ordering", () => {
    it("should display traits in priority order", () => {
      ...
      // Expand Traits traits to see all metadata
      const showAllButton = screen.queryByText("Expand Traits");
      if (showAllButton) {
        fireEvent.click(showAllButton);
      }
  ...
  describe("expand/collapse traits functionality", () => {
    it('should show "Expand Traits" button when more than 2 traits exist', () => {
      const drop = createMockDrop([
      ...
      expect(screen.getByText("Expand Traits")).toBeInTheDocument();
      expect(screen.queryByText("Collapse Traits")).not.toBeInTheDocument();
    });

    it('should expand to show all traits when "Expand Traits" is clicked', () => {
      const drop = createMockDrop([
        createMetadata("artist", "Artist 1"),
        createMetadata("palette", "Palette 1"),
        createMetadata("style", "Style 1"),
      ...
      expect(screen.getByText("Collapse Traits")).toBeInTheDocument();
      expect(screen.queryByText("Expand Traits")).not.toBeInTheDocument();
    });

    it('should collapse to show only first 2 traits when "Collapse Traits" is clicked', () => {
      const drop = createMockDrop([
        createMetadata("artist", "Artist 1"),
        createMetadata("palette", "Palette 1"),
        createMetadata("style", "Style 1"),
      ...
      // Collapse
      fireEvent.click(screen.getByText("Collapse Traits"));
    ...
    it('should not show "Expand Traits" button when only 2 or fewer traits exist', () => {
      const drop = createMockDrop([createMetadata("artist", "Artist 1")], ""); // Only one string trait, plus default boolean traits
      drop.parts = [];

      render(<DropTraits drop={drop} />);

      // With only one custom trait plus boolean traits, might still have "Expand Traits"
      // Let's check if there are 2 or fewer total meaningful traits
      const showAllButton = screen.queryByText("Expand Traits");
      const showLessButton = screen.queryByText("Collapse Traits");

      // Either no "Expand Traits" button OR it's present but we have valid traits
      expect(showLessButton).not.toBeInTheDocument();
    });
  });

  describe("event handling", () => {
    it("should stop propagation on expand traits button click", () => {
      const drop = createMockDrop(
        [
          createMetadata("artist", "Artist 1"),
          createMetadata("palette", "Palette 1"),
      ...
      const showAllButton = screen.getByText("Expand Traits");
    ...
    it("should stop propagation on collapse traits button click", () => {
      const drop = createMockDrop(
        [
          createMetadata("artist", "Artist 1"),
          createMetadata("palette", "Palette 1"),
      ...
      const showLessButton = screen.getByText("Collapse Traits");
