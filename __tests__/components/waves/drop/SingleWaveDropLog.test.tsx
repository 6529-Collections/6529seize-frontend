...
const mobileMock = require("@/hooks/useIsMobileLayoutViewport")
...
jest.mock("@/components/common/SystemAdjustmentPill", () => ({
  SystemAdjustmentPill: () => <span data-testid="pill" />,
}));

describe("SingleWaveDropLog", () => {
  const baseLog: any = {
    created_at: Date.now(),
    invoker: { handle: "0xabc", pfp: "" },
    contents: { oldVote: 0, newVote: 1 },
    id: "1",
  };
  it("renders desktop layout with pill", () => {
    render(
      <SingleWaveDropLog
        log={{
          ...baseLog,
          contents: { oldVote: 1, newVote: 2, reason: "CREDIT_OVERSPENT" },
        }}
  ...
  it("shows avatar placeholder when no pfp and mobile", () => {
    ...
    const { container } = render(
      <SingleWaveDropLog log={baseLog} creditType={ApiWaveCreditType.Rep} />
    );
