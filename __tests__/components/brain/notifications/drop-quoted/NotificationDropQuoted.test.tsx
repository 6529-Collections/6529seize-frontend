...
jest.mock(
  ...
  () => ({
    ...
    default: ({ children }: { children: React.ReactNode }) => (
      <div data-testid="notification-header">{children}</div>
    ),
  })
);
...
jest.mock("@/components/waves/drops/Drop", () => ({
  __esModule: true,
  default: (props: {
    onReplyClick: (n: number) => void;
    onQuoteClick: (q: unknown) => void;
  }) => (
    <div>
      <button onClick={() => props.onReplyClick(11)}>reply</button>
      <button
        onClick={() => props.onQuoteClick({ wave: { id: "q" }, serial_no: 22 })}
      >
        quote
      </button>
    </div>
...
describe("NotificationDropQuoted", () => {
  ...
  const notification = {
    related_drops: [
      { wave: { id: "w" }, serial_no: 5, author: { handle: "alice" } },
    ],
    created_at: Date.now(),
  } as never;
  ...
  it("navigates on reply click", async () => {
    ...
    expect(push).toHaveBeenCalledWith("/waves/w?serialNo=11");
  });
  ...
  it("navigates on quote click", async () => {
    ...
    expect(push).toHaveBeenCalledWith("/waves/q?serialNo=22");
  });
});
