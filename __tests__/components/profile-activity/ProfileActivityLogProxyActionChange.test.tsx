...
test('displays end time change with formatted date', () => {
  ...
  expect((CommonProfileLink as jest.Mock).mock.calls[0][0].isCurrentUser).toBe(true);
});

test('displays credit amount change', () => {
  (useRouter as jest.Mock).mockReturnValue({ query: { user: '' } });
  const log: any = {
    target_profile_handle: 'Carol',
    contents: { type: ApiProfileProxyActionType.AllocateCic, credit_amount: 1000 },
  };
  render(<ProfileActivityLogProxyActionChange log={log} />);
