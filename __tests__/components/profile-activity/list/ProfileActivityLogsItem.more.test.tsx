...
describe("ProfileActivityLogsItem additional cases", () => {
  it("renders banner component for banner edits", () => {
    ...
    expect(
      document.querySelector('[data-testid="BANNER"]')
    ).toBeInTheDocument();
    rerender(
      <ProfileActivityLogsItem
        log={{ ...base, type: ProfileActivityLogType.BANNER_2_EDIT }}
        user={null}
      />
    );
  ...
  it("renders proxy action state component", () => {
    render(
      <ProfileActivityLogsItem
        log={{ type: ProfileActivityLogType.PROXY_ACTION_STATE_CHANGED } as any}
        user={null}
      />
    );
