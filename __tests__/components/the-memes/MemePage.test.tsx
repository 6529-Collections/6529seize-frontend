...
jest.mock('../../../components/the-memes/MemePageLive', () => ({
  MemePageLiveRightMenu: ({ show }: any) => (show ? <div data-testid="live-right">Live</div> : null),
  MemePageLiveSubMenu: ({ show }: any) => (show ? <div data-testid="live-sub">Live Sub</div> : null),
}));
...
jest.mock('../../../components/the-memes/MemePageActivity', () => ({
  MemePageActivity: ({ show }: any) => (show ? <div data-testid="activity">Activity</div> : null),
}));

jest.mock('../../../components/the-memes/MemePageArt', () => ({
  MemePageArt: ({ show }: any) => (show ? <div data-testid="art">Art</div> : null),
}));

jest.mock('../../../components/the-memes/MemePageTimeline', () => ({
  MemePageTimeline: ({ show }: any) => (show ? <div data-testid="timeline">Timeline</div> : null),
}));
...
describe('MemePage tab navigation', () => {
  ...
  ])('selecting %s shows component and updates query', async (label, focus, testId) => {
    renderPage();
    await waitFor(() => expect(screen.getByTestId('mint-countdown')).toBeInTheDocument());
