...
describe("MemePageActivity", () => {
  ...
  describe("Component Rendering", () => {
    it("renders nothing when show is false", () => {
      const { container } = render(<MemePageActivity show={false} nft={nft} pageSize={10} />);
      expect(container.firstChild).toBeNull();
    });
    
    it("renders nothing when show is true but nft is undefined", () => {
      const { container } = render(<MemePageActivity show nft={undefined} pageSize={10} />);
      expect(container.firstChild).toBeNull();
    });
  ...
  it("tests all TypeFilter values", async () => {
    ...
    for (const { filter, param } of filterTypes) {
      ...
      await waitFor(() => {
        expect(fetchUrlMock).toHaveBeenLastCalledWith(
          `https://api.test/api/transactions?contract=${MEMES_CONTRACT}&id=1&page_size=5&page=1&filter=${param}`
        );
      });
    }
  });
  ...
  it("resets page to 1 when filter changes", async () => {
    ...
    await waitFor(() => {
      expect(fetchUrlMock).toHaveBeenLastCalledWith(
        `https://api.test/api/transactions?contract=${MEMES_CONTRACT}&id=1&page_size=10&page=1&filter=sales`
      );
    });
  });
  ...
  describe("Pagination", () => {
    it("only shows pagination when there are activity results", async () => {
      ...
      const { unmount } = render(<MemePageActivity show nft={nft} pageSize={10} />);
