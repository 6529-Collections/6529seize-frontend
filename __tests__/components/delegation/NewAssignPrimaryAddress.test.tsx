...
jest.mock("@/components/delegation/DelegationFormParts", () => ({
  DelegationFormShell: (p: any) => (
    <section>
      <h2>{p.title}</h2>
      {p.children}
    </section>
  ),
  DelegationFormRow: (p: any) => <div>{p.children}</div>,
  DelegationFormField: (p: any) => <div>{p.children}</div>,
...
test("shows consolidation requirement message", () => {
  renderComp(
    ...
    {
      isFetching: false,
      data: { consolidation_key: "abc", address: "x", ens: "" },
    }
  );
