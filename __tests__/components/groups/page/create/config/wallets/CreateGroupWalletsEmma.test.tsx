...
jest.mock(
  "components/groups/page/create/config/wallets/GroupCreateWalletsCount",
  () => ({
    __esModule: true,
    default: ({ removeWallets }: any) => (
      <button aria-label="remove" onClick={removeWallets}>
        remove
      </button>
    ),
  })
  ...
  it("sets wallets from query data and removes them", () => {
    ...
    render(
      <AuthContext.Provider
        value={{ requestAuth, connectedProfile: { handle: "h" } } as any}
      >
    ...
    expect(screen.getByText("EMMA")).toHaveClass("tw-m-0");
