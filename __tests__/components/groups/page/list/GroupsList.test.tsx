...
jest.mock("react-use", () => ({
  useDebounce: (fn: any, _delay: number, deps: any[]) =>
    React.useEffect(fn, deps),
}));

jest.mock(
  "@/components/utils/infinite-scroll/CommonInfiniteScrollWrapper",
  () => (props: any) => {
    wrapperProps = props;
...
jest.mock(
  "@/components/groups/page/list/search/GroupsListSearch",
  () => (props: any) => {
    searchProps = props;
...
jest.mock(
  "@/components/groups/page/list/card/GroupCard",
  () => (props: any) => {
    cardProps.push(props);
    return <div data-testid={`card-${props.group.id}`} />;
  }
);
...
describe("GroupsList", () => {
  ...
  it("fetches next page when bottom reached with enough groups", () => {
    const groups = Array.from(
      { length: 21 },
      (_, i) => ({ id: `${i}`, created_at: i }) as ApiGroupFull
    );
  ...
  it("uses tighter row spacing without changing the column spacing", () => {
    const { container } = renderComponent([
      { id: "1", created_at: 1 } as ApiGroupFull,
    ]);

    expect(container.querySelector(".tw-grid")).toHaveClass(
      "tw-gap-x-4",
      "tw-gap-y-3",
      "md:tw-grid-cols-2",
      "lg:tw-gap-x-5",
