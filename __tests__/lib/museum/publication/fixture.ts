...
function buildBaseDocuments(): Record<string, string> {
  ...
  const museumPracticeMatrix = Object.fromEntries(
    Object.keys(useMatrix).map((action) => [
      action,
      {
        status: "ordinary_with_terms",
        note: `A specific Museum-practice reading for ${action}.`,
      },
    ])
  );
  ...
  documents[MUSEUM_RIGHTS_REGISTRY_PATH] = JSON.stringify({
    ...
    museum_practice_status_definitions: {
      ...
      ordinary_with_terms: "Ordinary with recorded terms.",
      purpose_limited: "Within the stated purpose.",
      contextual: "Context determines the reading.",
      separate_basis: "Separate permission or a legal exception.",
    },
    sources: {
