...
describe("wallet gallery snapshot normalization", () => {
  it("uses original indexed dimensions and emits a valid complete gallery", async () => {
    const wire = buildBackendSnapshot();
    wire.assets[0]!.metadata = { image_details: { width: 3000, height: 2000 } };
    const measure = jest.fn(async () => null);
    const snapshot = await enrichWalletGalleryImageDimensions(
      normalizeWalletGallerySnapshotResponse(wire, REQUESTED_SOURCES),
    ...
    const cmsPackage = buildCmsPackageCandidate({
      ...
      gallery: { ...state.gallery, snapshot },
    });
    expect(cmsPackage.payload.assets[0]).toEqual(
      expect.objectContaining({ width: 3000, height: 2000 })
    );
    expect(
  ...
  it("keeps NFT detail routes valid when every real snapshot image is unmeasurable", async () => {
    const snapshot = await enrichWalletGalleryImageDimensions(
      normalizeWalletGallerySnapshotResponse(
    ...
    expect(cmsPackage.payload.assets).toEqual([]);
    expect(
      cmsPackage.payload.pages.filter((page) => page.type === "nft_detail")
    ).toHaveLength(1);
    expect(
      cmsPackage.payload.routes.some((route) => route.path.includes("/nfts/"))
    ).toBe(true);
    expect(snapshot.warnings).toContain("backend_snapshot_unmeasured_images");
    expect(
      validateCmsPackageV1(cmsPackage, {
        allowFixtureSignatures: true,
        allowFixtureStorage: true,
  ...
  it("measures a preview rather than applying the original image's dimensions to different bytes", async () => {
    const wire = buildBackendSnapshot();
    wire.assets[0]!.media.image = null;
    wire.assets[0]!.media.image_preview = "https://media.6529.io/preview.png";
    wire.assets[0]!.metadata = { image_details: { width: 3000, height: 2000 } };
    const snapshot = await enrichWalletGalleryImageDimensions(
      normalizeWalletGallerySnapshotResponse(wire, REQUESTED_SOURCES),
      { measureImage: async () => ({ width: 800, height: 600 }) }
    );
