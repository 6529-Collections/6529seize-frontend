...
import { getServerSideProps as getProxyProps } from "@/pages/[user]/proxy";
import {
  getCommonHeaders,
  getUserProfile,
...
describe("user pages getServerSideProps", () => {
  ...
  it("returns props when no redirect needed", async () => {
    ...
    expect(result).toEqual({
      props: { profile: mockProfile, metadata: "meta" },
    });
  });

  it("proxy page returns props", async () => {
    (userPageNeedsRedirect as jest.Mock).mockReturnValue(false);
    const result = await getProxyProps(
      { query: { user: "bob" } } as any,
      null as any,
      null as any
    );
