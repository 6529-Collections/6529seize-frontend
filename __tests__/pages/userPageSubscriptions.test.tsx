...
describe("subscriptions page via createUserTabPage", () => {
  beforeEach(() => {
  ...
  it("renders layout + tab when no redirect needed", async () => {
    ...
    expect(await screen.findByTestId("subscriptions")).toHaveTextContent(
      "SUBS:alice"
    );
  ...
  it("redirects when userPageNeedsRedirect returns destination", async () => {
    (userPageNeedsRedirect as jest.Mock).mockReturnValue({
      redirect: { destination: "/bob/subscriptions" },
    });
    ...
    expect(redirectMock).toHaveBeenCalledWith("/bob/subscriptions");
  });
