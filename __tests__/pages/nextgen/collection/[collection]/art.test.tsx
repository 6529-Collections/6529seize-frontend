...
import Page, {
  getServerSideProps,
} from "../../../../../pages/nextgen/collection/[collection]/art";
...
jest.mock(
  ...
  () => ({
    ...
    getServerSideCollection: jest.fn(() =>
      Promise.resolve({ props: { collection: { name: "c" } } })
    ),
  })
);
...
describe("NextGen collection art page", () => {
  ...
  it("passes collection to child components", () => {
    const props = { collection: { id: 1, name: "Cool" } } as any;
    render(<Page {...props} />);
    expect(headerProps).toEqual({ collection: props.collection });
    expect(listProps).toEqual({ collection: props.collection });
  });
