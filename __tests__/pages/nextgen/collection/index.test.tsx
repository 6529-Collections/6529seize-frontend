...
import { getServerSideProps, useShallowRedirect } from '../../../../pages/nextgen/collection/[collection]/[[...view]]/index';
import { ContentView } from '../../../../components/nextGen/collections/collectionParts/NextGenCollection';

jest.mock('../../../../helpers/server.helpers', () => ({ getCommonHeaders: jest.fn(() => ({ h: '1' })) }));
...
describe('nextgen collection page', () => {
  it('fetches collection and returns props', async () => {
    (commonApiFetch as jest.Mock).mockResolvedValue({ name: 'Cool', banner: 'b' });
  ...
  it('useShallowRedirect replaces url', () => {
    ...
    (useRouter as jest.Mock).mockReturnValue({ asPath: '/nextgen/collection/1', replace });
    function Comp() { useShallowRedirect('Cool'); return null; }
    render(<Comp />);
    expect(replace).toHaveBeenCalledWith({ pathname: '/nextgen/collection/cool' }, undefined, { shallow: true });
  });
});
