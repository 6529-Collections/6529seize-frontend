...
describe('nextgen token page getServerSideProps', () => {
  it('fetches token, traits and collection', async () => {
    mockedFetch.mockImplementation(async ({ endpoint }) => {
      if (endpoint === 'nextgen/tokens/1') return { id: 1, name: 'Tok', collection_id: 2 };
      if (endpoint === 'nextgen/tokens/1/traits') return [{ token_count: 5 }];
      if (endpoint === 'nextgen/collections/2') return { id: 2, name: 'Coll', banner: 'b' };
    });

    const res = await getServerSideProps({ query: { token: 1, view: ['provenance'] }, req: {} } as any, {} as any, '/p');
  ...
  it('redirects when collection empty', async () => {
    mockedFetch.mockResolvedValueOnce(null).mockResolvedValueOnce({}).mockResolvedValueOnce({});
    mockedEmpty.mockReturnValue(true);
    const res = await getServerSideProps({ query: { token: 5 }, req: {} } as any, {} as any, '/p');
