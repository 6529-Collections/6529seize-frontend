...
jest.mock('../../components/user/stats/UserPageStats', () => ({ __esModule:true, default: ({ profile }: any) => <div data-testid="stats">{profile.handle}</div> }));
...
jest.mock('../../helpers/Helpers', () => ({ getMetadataForUserPage: jest.fn(() => 'meta') }));
  ...
  it('renders stats component and sets profile', () => {
    ...
    render(
      <ReactQueryWrapperContext.Provider value={{ setProfile } as any}>
        <Page pageProps={{ profile:{ handle:'alice' } } as any} />
      </ReactQueryWrapperContext.Provider>
    );
    expect(setProfile).toHaveBeenCalledWith({ handle:'alice' });
    expect(document.querySelector('[data-testid="stats"]')?.textContent).toBe('alice');
  });
