...
describe("seize Bootstrap theme", () => {
  const css = compile(path.join(process.cwd(), "styles/seize-bootstrap.scss"), {
  ...
  it("preserves brand colors in Bootstrap's derived CSS", () => {
    ...
    expect(css).toMatch(
      /\.btn-primary\s*\{[\s\S]*--bs-btn-bg: #267c93;[\s\S]*--bs-btn-border-color: #267c93;/
    );
    expect(css).toMatch(
      /\.text-bg-primary\s*\{[\s\S]*background-color: RGBA\(var\(--bs-primary-rgb\), var\(--bs-bg-opacity, 1\)\) !important;/
    );
  });
});
