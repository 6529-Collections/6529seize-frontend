...
// Mock dependencies
jest.mock('wagmi', () => ({
  createConnector: jest.fn((factory) => factory({ emitter: { emit: jest.fn() } })),
}));
...
describe('wagmiAppWalletConnector', () => {
  ...
  beforeEach(() => {
    ...
    mockCreateConnector.mockImplementation((factory) => factory({ emitter: mockEmitter }));
  ...
  describe('createAppWalletConnector', () => {
    it('creates connector with correct properties', () => {
      ...
      expect(connector.icon).toBe(`https://robohash.org/${mockAppWallet.address}.png`);
    ...
    it('throws error when no valid account found after decryption', async () => {
      const connector = createAppWalletConnector(
        { appWallet: mockAppWallet },
        mockRequestPasswordModal
      );
    ...
    it('returns empty accounts when no decrypted key', async () => {
      ...
      try {
        ...
        expect(error.message).toBe('No decrypted key found. Call connect() first.');
      }
    });
    ...
    it('throws error when switching to unknown chain', async () => {
      ...
      await expect(connector.switchChain({ chainId: 999 }))
