...
describe("wagmiAppWalletConnector", () => {
  ...
  const mockRequestPasswordModal = jest.fn() as jest.MockedFunction<
    () => Promise<string>
  >;
  let connector: any;
  ...
  describe("Capacitor environment tests", () => {
    ...
    it("throws InvalidPasswordError when address mismatch in Capacitor", async () => {
      ...
      ).rejects.toThrow("Password does not match wallet");
    });
  });

  describe("setPassword", () => {
    it("throws InvalidPasswordError when password is empty", async () => {
      await expect(connectorInstance.setPassword("")).rejects.toThrow(
        "Password is required and must be a string"
      );
    });

    it("throws InvalidPasswordError when password is null", async () => {
      await expect(connectorInstance.setPassword(null as any)).rejects.toThrow(
        "Password is required and must be a string"
      );
    });

    it("throws InvalidPasswordError when password is not a string", async () => {
      await expect(connectorInstance.setPassword(123 as any)).rejects.toThrow(
        "Password is required and must be a string"
      );
    });
    ...
    it("throws InvalidPasswordError when password decryption fails (short password case)", async () => {
      ...
      await expect(connectorInstance.setPassword("short")).rejects.toThrow(
    ...
    it("wraps unexpected errors in PrivateKeyDecryptionError", async () => {
      ...
      await expect(
        connectorInstance.setPassword("validpass123")
      ).rejects.toThrow("Unexpected error during password validation");
    });
  ...
  describe("connect", () => {
    it("throws error for unsupported chainId", async () => {
      ...
      await expect(
        connectorInstance.connect({ chainId: unsupportedChainId })
      ).rejects.toThrow(
        `Chain ID ${unsupportedChainId} is not supported. Supported chains: 1, 137`
      );
    });
  ...
  describe("connector properties", () => {
    it("has correct icon URL based on wallet address", () => {
      expect(connectorInstance.icon).toBe(
        `https://robohash.org/${mockAppWallet.address}.png?size=64x64`
      );
      expect(connectorInstance.iconUrl).toBe(
        `https://robohash.org/${mockAppWallet.address}.png?size=64x64`
      );
    });
    ...
    it("has correct name format", () => {
      const shortAddress =
        mockAppWallet.address.slice(0, 6) +
        "..." +
        mockAppWallet.address.slice(-4);
      expect(connectorInstance.name).toBe(
        `${mockAppWallet.name} (${shortAddress})`
      );
    });
  ...
  describe("getAccounts", () => {
    it("throws error when no decrypted private key available", async () => {
      await expect(connectorInstance.getAccounts()).rejects.toThrow(
  ...
  describe("getProvider and getClient", () => {
    it("throws error when no decrypted private key available", async () => {
      ...
      await expect(connectorInstance.getClient()).rejects.toThrow(
        "No decrypted key found. Call connect() first."
      );
    });
  });
  ...
  describe("switchChain", () => {
    it("throws error for unsupported chain", async () => {
      const unsupportedChainId = 999;
      await expect(
        connectorInstance.switchChain({ chainId: unsupportedChainId })
