...
describe("useNotificationsQuery", () => {
  ...
  it("flattens and reverses data", () => {
    ...
    const { result } = renderHook(() =>
      useNotificationsQuery({ identity: "id", reverse: true })
    );
  ...
  it("returns empty when no data", () => {
    ...
    const { result } = renderHook(() =>
      useNotificationsQuery({ identity: "id" })
    );
  ...
  it("shares coverage exclusions between prefetch and the live query", () => {
    useInfiniteQueryMock.mockReturnValue({
      data: undefined,
      isSuccess: false,
      isError: false,
    });
    const causeExclude = [ApiNotificationCause.SubscriptionCoverage];
    renderHook(() =>
      useNotificationsQuery({ identity: "alice", causeExclude })
    );
    const expectedKey = getIdentityNotificationsQueryKey({
      identity: "alice",
      causeExclude,
    });
    expect(useInfiniteQueryMock).toHaveBeenCalledWith(
