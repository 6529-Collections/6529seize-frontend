...
test("maps memes phase from roots using the onchain merkle root", async () => {
  ...
  expect(mockGetMemesMintingRoots).toHaveBeenCalledTimes(1);
});

test("uses the scheduled phase without fetching roots when disabled", async () => {
  const [phase0] = buildMemesPhases(
    Time.fromString("2024-07-03T00:00:00.000Z")
  );
  ...
  const { result } = renderHook(() =>
    useManifoldClaim({
      ...
      abi: [],
      identifier: 1,
      fetchMemesRoots: false,
    })
  );

  await waitFor(() => expect(result.current.claim?.memePhase?.id).toBe("0"));
  expect(result.current.claim?.memePhase?.name).toBe(
    "Phase 0 (Allowlist)"
  );
  expect(result.current.claim?.isDropComplete).toBe(false);
  expect(mockGetMemesMintingRoots).not.toHaveBeenCalled();
});

test("keeps polling an unresolved allowlist with a custom end time", async () => {
  const memePhases = buildMemesPhases(
    Time.fromString("2024-07-03T00:00:00.000Z")
  );
  const phase0 = memePhases[0];
  const publicPhase = memePhases.find((phase) => phase.id === "public");

  if (!phase0 || !publicPhase) {
    throw new Error("Expected the configured memes phases");
  }

  const customEnd = publicPhase.end.plusMinutes(30);
  jest.spyOn(Date, "now").mockReturnValue(customEnd.plusMinutes(1).toMillis());
  mockRead.mockReturnValue({
    data: [
      BigInt(10),
      {
        ...
        cost: BigInt(0),
        startDate: BigInt(phase0.start.toSeconds()),
        endDate: BigInt(customEnd.toSeconds()),
  ...
  await waitFor(() => expect(result.current.claim).toBeDefined());
  expect(result.current.claim?.memePhase).toBeUndefined();
  expect(result.current.claim?.isDropComplete).toBe(false);
  expect(mockRead.mock.lastCall?.[0].query.enabled).toBe(true);
  expect(mockGetMemesMintingRoots).not.toHaveBeenCalled();
});
