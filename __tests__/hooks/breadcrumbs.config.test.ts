...
describe('breadcrumbs.config', () => {
  ...
  describe('gradient config', () => {
    const gradientConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'gradient')!;
    ...
    it('should extract gradient id parameter', () => {
      const result = gradientConfig.paramExtractor(['6529-gradient', '123'], {});
    ...
    it('should build correct crumbs when loading', () => {
      const crumbs = gradientConfig.crumbBuilder({ id: '123' }, null, true, ['6529-gradient', '123'], {});
    ...
    it('should build correct crumbs with data', () => {
      const crumbs = gradientConfig.crumbBuilder({ id: '123' }, { name: 'Test Gradient' }, false, ['6529-gradient', '123'], {});
      expect(crumbs).toEqual([
        { display: '6529 Gradient', href: '/6529-gradient' },
    ...
    it('should build correct crumbs without data', () => {
      const crumbs = gradientConfig.crumbBuilder({ id: '123' }, null, false, ['6529-gradient', '123'], {});
      expect(crumbs).toEqual([
        { display: '6529 Gradient', href: '/6529-gradient' },
  ...
  describe('profile config', () => {
    const profileConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'profile')!;
    ...
    it('should build correct crumbs with data', () => {
      const crumbs = profileConfig.crumbBuilder({ handle: 'testuser' }, { handle: 'TestUser' }, false, ['profile', 'testuser'], {});
      expect(crumbs).toEqual([{ display: 'TestUser' }]);
    });

    it('should build correct crumbs without data', () => {
      const crumbs = profileConfig.crumbBuilder({ handle: 'testuser' }, null, false, ['profile', 'testuser'], {});
      expect(crumbs).toEqual([{ display: 'testuser' }]);
    });
  });
  ...
  describe('meme config', () => {
    ...
    it('should build correct crumbs for mint page', () => {
      const crumbs = memeConfig.crumbBuilder({ id: 'mint' }, null, false, ['the-memes', 'mint'], {});
      expect(crumbs).toEqual([
    ...
    it('should build correct crumbs with data', () => {
      const crumbs = memeConfig.crumbBuilder({ id: '123' }, { name: 'Test Meme' }, false, ['the-memes', '123'], {});
      expect(crumbs).toEqual([
        { display: 'The Memes', href: '/the-memes' },
        { display: 'Test Meme' }
      ]);
    ...
    it('should build correct crumbs without data', () => {
      const crumbs = memeConfig.crumbBuilder({ id: '123' }, null, false, ['the-memes', '123'], {});
      expect(crumbs).toEqual([
        { display: 'The Memes', href: '/the-memes' },
        { display: 'Meme #123' }
      ]);
  ...
  describe('nextgen config', () => {
    const nextgenConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'nextgen')!;
    ...
    it('should extract nextgen id parameter from third segment', () => {
      const result = nextgenConfig.paramExtractor(['nextgen', 'collection', '123'], {});
      expect(result).toEqual({ id: '123' });
    });
    ...
    it('should build correct crumbs with data', () => {
      const crumbs = nextgenConfig.crumbBuilder({ id: '123' }, { name: 'Test Collection' }, false, ['nextgen', 'collection', '123'], {});
      expect(crumbs).toEqual([
  ...
  describe('rememe config', () => {
    const rememeConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'rememe')!;
    ...
    it('should extract rememe contract and id parameters', () => {
      const result = rememeConfig.paramExtractor(['rememes', '0x123', '456'], {});
    ...
    it('should build correct query key', () => {
      const queryKey = rememeConfig.queryKeyBuilder({ contract: '0x123', id: '456' });
      expect(queryKey).toEqual(['breadcrumb', 'rememe', '0x123', '456']);
    });

    it('should build correct crumbs with data', () => {
      const crumbs = rememeConfig.crumbBuilder({ contract: '0x123', id: '456' }, { name: 'Test Rememe' }, false, ['rememes', '0x123', '456'], {});
      expect(crumbs).toEqual([
  ...
  describe('meme-lab config', () => {
    const memeLabConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'meme-lab')!;
    ...
    it('should build correct crumbs with data', () => {
      const crumbs = memeLabConfig.crumbBuilder({ id: '123' }, { name: 'Test Lab' }, false, ['meme-lab', '123'], {});
      expect(crumbs).toEqual([
    ...
    it('should build correct crumbs without data', () => {
      const crumbs = memeLabConfig.crumbBuilder({ id: '123' }, null, false, ['meme-lab', '123'], {});
      expect(crumbs).toEqual([
        { display: 'Meme Lab', href: '/meme-lab' },
        { display: 'Card 123' }
      ]);
  ...
  describe('collection config', () => {
    const collectionConfig = DYNAMIC_ROUTE_CONFIGS.find(c => c.type === 'collection')!;
    ...
    it('should extract collection id parameter', () => {
      const result = collectionConfig.paramExtractor(['collections', '123'], {});
      expect(result).toEqual({ id: '123' });
    });

    it('should build correct crumbs with data', () => {
      const crumbs = collectionConfig.crumbBuilder({ id: '123' }, { name: 'Test Collection' }, false, ['collections', '123'], {});
      expect(crumbs).toEqual([
  ...
  describe('wave config', () => {
    ...
    it('should build correct crumbs for waves', () => {
      const crumbs = waveConfig.crumbBuilder({ id: '123' }, { name: 'Test Wave' }, false, ['waves', '123'], {});
      expect(crumbs).toEqual([
    ...
    it('should build correct crumbs for my-stream', () => {
      const crumbs = waveConfig.crumbBuilder({ id: '123' }, { name: 'Test Wave' }, false, ['my-stream', '123'], {});
      expect(crumbs).toEqual([
    ...
    it('should build correct crumbs without data', () => {
      const crumbs = waveConfig.crumbBuilder({ id: '123' }, null, false, ['waves', '123'], {});
      expect(crumbs).toEqual([
        { display: 'Waves', href: '/waves' },
        { display: 'Wave 123' }
      ]);
