...
describe('useActivityData', () => {
  ...
  describe('API Calls', () => {
    it('makes API call with correct base URL and pagination parameters', async () => {
      ...
      expect(mockFetchUrl).toHaveBeenCalledWith(
        'https://test-api.6529.io/api/transactions?page_size=20&page=1'
      );
    });
  ...
  describe('Type Filters', () => {
    it('adds sales filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=sales'
        );
      });
    });
    ...
    it('adds transfers filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=transfers'
        );
      });
    });
    ...
    it('adds airdrops filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=airdrops'
        );
      });
    });
    ...
    it('adds mints filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=mints'
        );
      });
    });
    ...
    it('adds burns filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=burns'
        );
      });
    });
  ...
  describe('Contract Filters', () => {
    it('adds memes contract filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          `https://test-api.6529.io/api/transactions?page_size=20&page=1&contract=${MEMES_CONTRACT}`
        );
      });
    });
    ...
    it('adds nextgen contract filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          `https://test-api.6529.io/api/transactions?page_size=20&page=1&contract=0xNextGenCore`
        );
      });
    });
    ...
    it('adds gradients contract filter to URL', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          `https://test-api.6529.io/api/transactions?page_size=20&page=1&contract=${GRADIENT_CONTRACT}`
        );
      });
    });
  ...
  describe('Combined Filters', () => {
    it('combines type and contract filters correctly', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          `https://test-api.6529.io/api/transactions?page_size=20&page=1&filter=sales&contract=${MEMES_CONTRACT}`
        );
      });
    });
    ...
    it('handles multiple filters with different combinations', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          `https://test-api.6529.io/api/transactions?page_size=10&page=1&filter=transfers&contract=0xNextGenCore`
        );
      });
    });
  });
  ...
  describe('Pagination', () => {
    ...
    it('fetches new data when page changes', async () => {
      ...
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=20&page=2'
        );
      });
    });
    ...
    it('uses different page sizes correctly', async () => {
      renderHook(() => 
        useActivityData(1, 50, TypeFilter.ALL, 'All' as any)
      );
      
      await waitFor(() => {
        expect(mockFetchUrl).toHaveBeenCalledWith(
          'https://test-api.6529.io/api/transactions?page_size=50&page=1'
        );
      });
    });
  });
  ...
  describe('Initial Data Optimization', () => {
    ...
    it('fetches data when initial data is provided but filters are not default', async () => {
      ...
      renderHook(() => 
        useActivityData(1, 20, TypeFilter.SALES, ContractFilter.ALL, initialData)
      );
  ...
  describe('Effect Dependencies', () => {
    ...
    it('refetches when selectedContract changes', async () => {
      const { rerender } = renderHook(
        ({ selectedContract }) => useActivityData(1, 20, TypeFilter.ALL, selectedContract),
    ...
    it('refetches when pageSize changes', async () => {
      const { rerender } = renderHook(
        ({ pageSize }) => useActivityData(1, pageSize, TypeFilter.ALL, 'All' as any),
  ...
  describe('Environment Configuration', () => {
    ...
    it('uses API_ENDPOINT environment variable correctly', async () => {
      ...
      try {
        ...
        // Override default mock with specific response for this test
        mockFetchUrl.mockResolvedValueOnce({ count: 0, page: 1, next: null, data: [] } as DBResponse);
