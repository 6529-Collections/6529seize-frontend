...
const validateSecurityEventContext = (context: SecurityEventContext): void => {
  ...
  if (/0x[a-fA-F0-9]{40}/.test(contextJson)) {
    throw new Error('SECURITY VIOLATION: SecurityEventContext contains wallet address');
  }
  
  // Check for partial addresses that could be used for fingerprinting (0x + 20+ hex chars)
  if (/0x[a-fA-F0-9]{20,39}/.test(contextJson)) {
    throw new Error('SECURITY VIOLATION: SecurityEventContext contains potential token/key');
  }
  ...
  if (/eyJ[A-Za-z0-9-_]+/.test(contextJson)) {
    throw new Error('SECURITY VIOLATION: SecurityEventContext contains JWT token');
  }
};
 ...
 */
export const createConnectionEventContext = (source: string): SecurityEventContext => ({
