...
export function WebSocketProvider({ children, config }: WebSocketProviderProps) {
  ...
  const attemptReconnect = useCallback(() => {
    ...
    if (reconnectAttemptsRef.current >= maxAttempts) {
      console.warn(`WebSocket reconnect failed after ${reconnectAttemptsRef.current} attempts`);
      return;
    }
   ...
   */
  const subscribe = useCallback(<T,>(messageType: string, callback: MessageCallback<T>) => {
