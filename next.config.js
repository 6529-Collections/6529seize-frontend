...
const nextConfig = {
  ...
  env: {
    ...
    CORE_SCHEME: env.CORE_SCHEME,
    IPFS_API_ENDPOINT: env.IPFS_API_ENDPOINT,
    IPFS_GATEWAY_ENDPOINT: env.IPFS_GATEWAY_ENDPOINT,
    IPFS_MFS_PATH: env.IPFS_MFS_PATH,
    TENOR_API_KEY: env.TENOR_API_KEY,
    WS_ENDPOINT: env.WS_ENDPOINT,
    DEV_MODE_MEMES_WAVE_ID: env.DEV_MODE_MEMES_WAVE_ID,
    DEV_MODE_WALLET_ADDRESS: env.DEV_MODE_WALLET_ADDRESS,
    DEV_MODE_AUTH_JWT: env.DEV_MODE_AUTH_JWT,
    USE_DEV_AUTH: env.USE_DEV_AUTH,
    STAGING_API_KEY: env.STAGING_API_KEY,
    AWS_RUM_APP_ID: env.AWS_RUM_APP_ID,
    AWS_RUM_REGION: env.AWS_RUM_REGION,
    AWS_RUM_SAMPLE_RATE: env.AWS_RUM_SAMPLE_RATE,
  },
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    if (!dev && !isServer) {
      const requiredEnvVars = ["BASE_ENDPOINT"];
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `BASE_ENDPOINT must use HTTPS in production: ${baseEndpoint}`
          );
        }
      } catch (error) {
