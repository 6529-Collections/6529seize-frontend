...
export default function DropVoteProgressing({
  ...
  if (subtle) {
    ...
    color = isPositiveProgressing
      ? "tw-text-emerald-400 tw-bg-emerald-500/10 tw-px-1.5 tw-rounded tw-border tw-border-solid tw-border-emerald-500/20 tw-font-mono"
      : "tw-text-rose-400 tw-bg-rose-500/10 tw-px-1.5 tw-rounded tw-border tw-border-solid tw-border-rose-500/20 tw-font-mono";
    arrowColor = "tw-text-iron-600";
  }
  ...
  return (
    <>
      ...
      >
        ...
        />
        <span className={`${subtle ? 'tw-text-xs' : 'tw-text-sm'} tw-font-medium ${color}`}>{formatNumberWithCommas(projected)}</span>
      </span>
      <Tooltip
