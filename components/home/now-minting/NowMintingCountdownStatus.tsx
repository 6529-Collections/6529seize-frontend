...
const STATUS_CONFIGS: Record<StatusType, StatusConfig> = {
  ...
  sold_out: {
    icon: <CheckCircleIcon className="tw-size-[30px] tw-flex-shrink-0 tw-text-emerald-300/80" />,
    iconBg: "tw-bg-iron-900/60",
    iconRing: "tw-ring-white/10",
    topLeftText: "Mint Complete",
    blackDivText: "All NFTs have been successfully minted.",
  },
  finalized: {
...
export default function NowMintingCountdownStatus({
  ...
  return (
    <div className="tw-relative tw-overflow-hidden tw-rounded-lg tw-border tw-border-solid tw-border-white/5 tw-bg-iron-900/40 tw-p-3 tw-text-left md:tw-p-4">
      ...
      <div className="tw-mb-3 tw-flex tw-h-14 tw-items-center tw-justify-center tw-gap-3 tw-rounded-md tw-border tw-border-white/10 tw-bg-iron-950 tw-shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)]">
        <div
      ...
      {type !== "error" && (
        <div className="tw-flex tw-h-10 tw-w-full tw-items-center tw-justify-center tw-gap-x-1.5 tw-whitespace-nowrap tw-rounded-lg tw-border-0 tw-bg-iron-200 tw-px-3 tw-py-2 tw-text-sm tw-font-semibold tw-text-iron-950">
