...
function ReviewWalletDetails({
  ...
  return (
    <div className="tw-min-w-0">
      <p id={labelId} className="tw-m-0 tw-text-xs tw-text-iron-400">
        {label}
      </p>
      <div className="tw-flex tw-min-w-0 tw-items-start tw-gap-2">
        <div id={identityId} className="tw-min-w-0 tw-flex-1 tw-py-1">
          {showName && (
            <bdi className="tw-block tw-break-words tw-text-sm tw-font-semibold tw-text-iron-100 [overflow-wrap:anywhere]">
              {displayName}
            </bdi>
          )}
          <code
