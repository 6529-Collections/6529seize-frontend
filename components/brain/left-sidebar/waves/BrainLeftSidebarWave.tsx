...
const BrainLeftSidebarWave: React.FC<BrainLeftSidebarWaveProps> = ({
  ...
  const haveNewDrops = wave.newDropsCount.count > 0 && !isActive;
  ...
  return (
    <div
      className={`tw-relative tw-flex tw-px-5 tw-py-2 tw-group tw-transition-colors tw-duration-200 tw-ease-in-out ${
        isActive
      ...
      {haveNewDrops && (
        <>
          <div className="tw-absolute tw-left-0 tw-top-1/2 tw-w-3 tw-h-3 tw-bg-white tw-rounded-full tw--translate-x-1/2 tw--translate-y-1/2" />
          {/* <div className="tw-absolute tw-left-[0.45rem] tw-top-1/2 tw--translate-y-1/2 tw-text-iron-300 tw-text-[9px]">
            {newDropsCountDisplay}
          </div> */}
        </>
      )}
      <Link
          ...
          <div
            className={`tw-text-sm ${
              haveNewDrops
                ...
                : "tw-font-medium tw-text-iron-300"
            }`}>
            {wave.name}
          </div>
