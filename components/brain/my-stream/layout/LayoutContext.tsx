...
export const LayoutProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // Define calculateSpaces at component level so it's available everywhere
  const calculateSpaces = useCallback(() => {
    ...
    const calculatedContentSpace = Math.max(
      0,
      viewportHeight - totalOccupiedSpace
    );
    ...
    if (isMountedRef.current && !abortControllerRef.current.signal.aborted) {
      ...
      setSpaces((prevSpaces) => {
        ...
        // For tabs, use a slightly relaxed comparison with small threshold to catch minor changes
        const tabsChanged = Math.abs(prevSpaces.tabsSpace - tabsHeight) > 1;
        ...
        // Debug tab height changes in development
        if (process.env.NODE_ENV === 'development' && tabsChanged) {
          console.log(`Tab height changed: ${prevSpaces.tabsSpace} -> ${tabsHeight}`);
        }
  ...
  }, [headerRef, pinnedRef, tabsRef, bottomRef]);
  ...
  const checkRefsReady = useCallback(() => {
    ...
    if (
      elementsReady.header &&
      elementsReady.pinned &&
      elementsReady.tabs &&
      elementsReady.bottom
    )
      return;
  ...
  useEffect(() => {
        ...
        frameIdRef.current = requestAnimationFrame(() => {
          ...
          setSpaces((prevSpaces) => {
            ...
            const significantChange =
              Math.abs(
                viewportHeight -
                  (prevSpaces.headerSpace +
                    prevSpaces.bottomSpace +
                    prevSpaces.pinnedSpace +
                    prevSpaces.tabsSpace) -
                  prevSpaces.contentSpace
    ...
    const eventListeners: Array<
      [string, EventListener, EventListenerOptions?]
    > = [];
    ...
    const focusHandler = (e: Event) => {
      ...
      if (
        ...
        (focusEvent.target instanceof HTMLElement &&
          focusEvent.target.isContentEditable)
      ) {
    ...
    if (
      ...
      try {
        if (resizeObserverSupported) {
          ...
          // Track event listeners to be cleaned up
          const resizeEventListeners: Array<
          ...
          const errorHandler = (e: Event) => {
            ...
            if (
              errorEvent.message &&
              errorEvent.message.includes("ResizeObserver loop")
            ) {
          ...
          resizeEventListeners.push(["error", errorHandler]);
