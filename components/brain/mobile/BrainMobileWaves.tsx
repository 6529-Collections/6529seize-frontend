...
// Temporary local fixture for visual review. Remove after Uncast Power refinement.
const LOCAL_UNCAST_POWER_PREVIEW_STATS: MemesWaveFooterStats = {
  isAvailable: true,
  isReady: true,
  leftThisRoundCount: 325,
  uncastPower: 1_720,
  unratedCount: 459,
  votingLabel: "TDH",
};
...
const BrainMobileWaves: React.FC<BrainMobileWavesProps> = ({
  ...
  const isLocalUncastPowerPreview =
    process.env.NODE_ENV === "development" && !footerStats.isAvailable;
  const displayedFooterStats = isLocalUncastPowerPreview
    ? LOCAL_UNCAST_POWER_PREVIEW_STATS
    : footerStats;
  const scrollClearanceClassName = displayedFooterStats.isAvailable
    ? MEMES_WAVE_FLOATING_FOOTER_SCROLL_CLEARANCE_CLASS_NAME
    : MEMES_WAVE_DOCK_ONLY_SCROLL_CLEARANCE_CLASS_NAME;
  ...
  return (
    <div
      className="tw-flex tw-h-full tw-min-h-0 tw-flex-col tw-bg-iron-950"
