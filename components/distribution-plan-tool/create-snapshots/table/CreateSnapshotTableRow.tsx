...
function CopyableValue({
  ...
  return (
    <button
      ...
      onClick={onCopy}
      className="tw-group tw-inline-flex tw-min-h-8 tw-items-center tw-rounded-md tw-border-0 tw-bg-transparent tw-p-0 tw-text-sm tw-font-normal tw-text-iron-300 tw-transition-colors tw-duration-200 focus-visible:tw-outline focus-visible:tw-outline-2 focus-visible:tw-outline-offset-2 focus-visible:tw-outline-primary-400 desktop-hover:hover:tw-text-white"
    >
      <span className={copied ? "tw-text-primary-400" : undefined}>
        {displayValue}
      </span>
      <svg
        aria-hidden="true"
        className="tw-ml-2 tw-size-5 tw-text-iron-400 tw-transition-colors tw-duration-200 desktop-hover:group-hover:tw-text-white"
        viewBox="0 0 24 24"
        fill="none"
...
export default function CreateSnapshotTableRow({
  ...
  return (
    <DistributionPlanTableRowWrapper>
      ...
      <td className="tw-whitespace-nowrap tw-px-3 tw-py-4">
        <CopyableValue
          displayValue={contractText}
      ...
      <td className="tw-whitespace-nowrap tw-px-3 tw-py-4">
        <CopyableValue
          displayValue={blockNoText}
          label="block number"
          copied={copiedField === "blockNo"}
          onCopy={copyBlockNumber}
        />
      </td>
      <td className="tw-whitespace-nowrap tw-px-3 tw-py-4 tw-text-sm tw-font-normal tw-text-iron-300">
        <div className="tw-flex tw-h-full tw-items-center">
          ...
          <button
            type="button"
            aria-label={`Show token IDs for ${snapshot.name}`}
            data-tooltip-id={`token-ids-tooltip-${snapshot.id}`}
            className="tw-ml-1 tw-inline-flex tw-size-8 tw-shrink-0 tw-items-center tw-justify-center tw-rounded-full tw-border-0 tw-bg-transparent tw-p-0 tw-text-iron-500 focus-visible:tw-outline focus-visible:tw-outline-2 focus-visible:tw-outline-primary-400 desktop-hover:hover:tw-text-iron-300"
          >
            <svg
              aria-hidden="true"
              className="tw-size-4"
      ...
      <td className="tw-whitespace-nowrap tw-px-3 tw-py-4">
        {haveConsolidatedBlockNo && (
          <CopyableValue
            displayValue={consolidatedBlockNoText}
            label="consolidation block number"
            copied={copiedField === "consolidatedBlockNo"}
            onCopy={copyConsolidatedBlockNumber}
          />
        )}
      </td>
      <td className="tw-whitespace-nowrap tw-px-3 tw-py-4 tw-text-sm tw-font-normal tw-text-iron-300">
