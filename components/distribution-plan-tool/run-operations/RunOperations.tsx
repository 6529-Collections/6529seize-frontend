...
export default function RunOperations() {
  const { distributionPlan, setState, fetching } = useContext(
  ...
  useEffect(() => {
    ...
    if (
      distributionPlan.activeRun?.status &&
      LOADING_STATES.includes(distributionPlan.activeRun.status)
    ) {
      setLoading(true);
    } else {
      setLoading(false);
    }
  }, [distributionPlan, fetching]);

  useEffect(() => setShowLoader(loading || fetching), [loading, fetching]);
  ...
  useInterval(
    ...
    distributionPlan?.id && loading ? 2000 : null
  );
  return (
    <>
      {showLoader && (
