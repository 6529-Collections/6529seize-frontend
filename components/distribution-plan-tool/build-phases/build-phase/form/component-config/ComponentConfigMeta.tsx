    ...
    <div className="tw-space-y-1 tw-self-center">
      {/* <div className="tw-flex tw-space-x-2">
        {tags.map((tag) => (
          <span
            key={tag.id}
            className="tw-font-normal tw-text-primary-400 tw-text-xs tw-truncate group-hover:tw-text-iron-400 tw-transition tw-duration-300 tw-ease-out"
          >
            {tag.name}
          </span>
        ))}
      ...
      <div className="tw-text-xs tw-space-x-2">
        <span className="tw-text-iron-400 tw-font-light">
          Total wallets:
        </span>
        {!isLoading ? (
          <span className="tw-font-normal">{walletsCount ?? "N/A"}</span>
        ) : (
          <svg
            aria-hidden="true"
            role="status"
