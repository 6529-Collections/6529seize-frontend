export default function DistributionPlanToolNotConnected() {
  return (
    <div className="tw-flex tw-flex-col">
      <h1 className="tw-uppercase tw-text-white">CONNECT YOUR WALLET</h1>
      <div className="tw-mb-6 lg:tw-max-w-2xl">
        <p className="tw-text-justify tw-mb-0 tw-mt-2 tw-text-base tw-leading-[1.6] tw-font-normal tw-text-neutral-300">
          Connect with an address from within your consolidated account to proceed, 
          so that we can verify your TDH to grant you access. 
          No special delegation is required. 
          Please don't connect your vault.
        </p>
        <p className="tw-text-justify tw-mb-0 tw-mt-2 tw-text-base tw-leading-[1.6] tw-font-normal tw-text-neutral-300">
          No gas is needed to sign in, and there is no cost or fee to use the tool.
        </p>
      </div>
    </div>
  );
}
