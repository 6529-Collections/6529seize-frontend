...
const TermsSignatureFlow: FC = () => {
  ...
  const [pendingCallback, setPendingCallback] = useState<((result: SigningCompleteResult) => void) | null>(null);
  ...
  useEffect(() => {
    ...
    document.addEventListener('showTermsModal', handleShowTermsModal as EventListener);
    
    // Clean up
    return () => {
      document.removeEventListener('showTermsModal', handleShowTermsModal as EventListener);
    };
  }, []);
