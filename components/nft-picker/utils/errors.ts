...
export function isRangeTooLargeError(
  value: unknown
): value is RangeTooLargeError {
  if (!(value instanceof Error)) {
    return false;
  }
  const candidate = value as Partial<RangeTooLargeError>;
  return (
    candidate.code === "range-too-large" && typeof candidate.size === "bigint"
  );
}
