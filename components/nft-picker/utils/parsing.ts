...
function parseTokenValue(
    ...
    if (!value) {
        errors.push(makeError(segment.value, segment.start, segment.end, "Token is empty"));
        return null;
    }
    if (/^[+-]/.test(value)) {
        errors.push(
...
function parseSegmentToRanges(segment: Segment, errors: ParseError[]): TokenRange[] {
    ...
    errors.push(
        makeError(segment.value, segment.start, segment.end, "Invalid range expression")
    );
    return [];
}
...
export function parseTokenExpressionToRanges(input: string): TokenRange[] {
    ...
    const total = canonical.reduce(
        (sum: bigint, range: TokenRange) => sum + (range.end - range.start + BIGINT_ONE),
        BIGINT_ZERO
    );
