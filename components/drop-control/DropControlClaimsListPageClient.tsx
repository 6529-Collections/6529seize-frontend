...
function ClaimCardThumbnail({ claim }: { claim: MemeClaim }) {
  ...
  return (
    <div className="tw-flex tw-w-20 tw-flex-shrink-0 tw-items-center tw-justify-center tw-self-stretch tw-rounded-lg tw-bg-iron-900 tw-text-iron-500 tw-ring-1 tw-ring-iron-800">
      —
    </div>
  );
}

interface DropControlClaimsPageClientProps {
  mode?: "prepare" | "launch";
}

export default function DropControlClaimsListPageClient({
  mode = "prepare",
}: DropControlClaimsPageClientProps) {
  ...
  const section =
    mode === "launch"
      ...
      : DROP_CONTROL_SECTIONS.PREPARE;
  const { title, description } = section;
  const hasAccess = mode === "launch" ? canAccessLaunchPage : canAccessPrepare;
  ...
  const fetchPage = useCallback(async (p: number) => {
    ...
    setError(null);
    try {
      const res = await getClaimsPage(p, PREPARE_CLAIMS_PAGE_SIZE);
      setData(res);
    } catch (e) {
  ...
  return (
    <div className="tw-px-2 tw-pb-16 tw-pt-2 lg:tw-px-6 lg:tw-pt-8 xl:tw-px-8">
      ...
      <p className="tw-mb-6 tw-text-iron-400">{description}</p>
      ...
      {data && (
        <>
          ...
          {data.count > PREPARE_CLAIMS_PAGE_SIZE && data.claims.length > 0 && (
            <div className="tw-mt-10 tw-flex tw-items-center tw-justify-center tw-gap-2">
              <Pagination
                page={page}
                pageSize={PREPARE_CLAIMS_PAGE_SIZE}
...
function ClaimCard({
  ...
  return <PrepareClaimCard claim={claim} />;
}

function getCardDetailsBasePath(mode: "prepare" | "launch"): string {
  return mode === "launch" ? "/drop-control/launch" : "/drop-control/prepare";
}
