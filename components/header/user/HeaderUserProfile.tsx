...
import { isAutoGeneratedHandle, isEthereumAddress } from "@/helpers/AllowlistToolHelpers";
...
export default function HeaderUserProfile({
  ...
  const matchingWallet = useMemo(() => 
    profile?.wallets?.find(w => w.wallet.toLowerCase() === address?.toLowerCase()),
    [profile?.wallets, address]
  );

  const label = useMemo(() => {
    if (activeProfileProxy) return activeProfileProxy.created_by.handle ?? "";
    if (profile?.handle) return profile.handle.slice(0, 20);
    if (matchingWallet?.display) {
      return isEthereumAddress(matchingWallet.display) || isAutoGeneratedHandle(matchingWallet.display) 
        ? matchingWallet.display.slice(0, 6) 
        : matchingWallet.display;
    }
    if (address) return address.slice(0, 6);
    throw new Error("No label found");
  }, [activeProfileProxy, profile?.handle, matchingWallet, address]);

  const pfpUrl = useMemo(() => {
    if (activeProfileProxy) return activeProfileProxy.created_by.pfp ?? null;
    return profile?.pfp ?? null;
  }, [activeProfileProxy, profile?.pfp]);

  const path = useMemo(() => {
    if (activeProfileProxy) return `/${activeProfileProxy.created_by.handle}`;
    if (profile?.handle) return `/${profile.handle}`;
    if (matchingWallet?.display) return `/${matchingWallet.display}`;
    if (address) return `/${address}`;
    throw new Error("No path found");
  }, [activeProfileProxy, profile?.handle, matchingWallet, address]);

  const isProxy = useMemo(() => !!activeProfileProxy, [activeProfileProxy]);

  const statusStyles = useMemo(() => ({
    backgroundColor: isConnected ? "rgb(0,220,33)" : "rgb(255,159,0)",
    boxShadow: `0 0 12px ${isConnected ? "rgba(0,220,33,1)" : "rgba(255,159,0,1)"}`,
  }), [isConnected]);
