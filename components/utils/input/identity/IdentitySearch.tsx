...
export default function IdentitySearch({
  ...
  const INPUT_CLASSES: Record<IdentitySearchSize, string> = {
    [IdentitySearchSize.SM]: "tw-py-2.5 tw-text-sm",
    [IdentitySearchSize.MD]: "tw-py-3 tw-text-base",
  };

  const SEARCH_ICON_CLASSES: Record<IdentitySearchSize, string> = {
    [IdentitySearchSize.SM]: "tw-top-2.5",
  ...
  return (
    <div className="tw-group tw-w-full tw-relative tw-z-20" ref={wrapperRef}>
      ...
      />
      <svg
        className={`${SEARCH_ICON_CLASSES[size]} tw-pointer-events-none tw-absolute tw-left-4 tw-h-5 tw-w-5 tw-text-iron-300`}
        viewBox="0 0 20 20"
        fill="currentColor"
        aria-hidden="true"
      >
