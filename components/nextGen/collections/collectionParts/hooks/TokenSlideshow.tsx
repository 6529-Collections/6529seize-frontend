...
export default function TokenSlideshow({ collectionId }: TokenSlideshowProps) {
  const swiperContainerRef = useRef<HTMLDivElement>(null);
  
  const { displayTokens, onSlideChange } = useTokenSlideshow(collectionId);
  const { slidesPerView } = useSlideshowConfig();
  const { isInViewport, setSwiperInstance } = useSlideshowAutoplay(swiperContainerRef);
  return (
    <Row>
      <Col>
