...
const formatWinningRanks = (
  outcome: CreateWaveOutcomeConfig
): string | null => {
  const ranks =
    outcome.winnersConfig?.winners.flatMap((winner, index) =>
      winner.value > 0 ? [index + 1] : []
    ) ?? [];
  ...
  const ranges: string[] = [];
  let rangeStart = firstRank;
  let rangeEnd = firstRank;
...
export default function CreateWaveOutcomesRowManualRank({
  ...
  const winningRanks = formatWinningRanks(outcome);
        ...
        <div className="tw-flex tw-flex-wrap tw-items-baseline tw-gap-x-3 tw-gap-y-1">
          ...
          {winningRanks ? (
            <span className="tw-text-xs tw-text-iron-400">
              {t(DEFAULT_LOCALE, "waves.create.outcomes.manual.savedRanks", {
                ranks: winningRanks,
              })}
            </span>
          ) : null}
