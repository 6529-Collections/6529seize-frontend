...
const WaveDropMobileMenu: FC<WaveDropMobileMenuProps> = ({
  ...
  const curationsDialogTimeoutRef = useRef<number | null>(null);

  useEffect(() => {
    return () => {
      if (curationsDialogTimeoutRef.current !== null) {
        window.clearTimeout(curationsDialogTimeoutRef.current);
      }
    };
  }, []);
  ...
  const handleCurationsClick = () => {
    ...
    curationsDialogTimeoutRef.current = window.setTimeout(() => {
      setIsCurationsDialogOpen(true);
      curationsDialogTimeoutRef.current = null;
    }, 250);
  };
