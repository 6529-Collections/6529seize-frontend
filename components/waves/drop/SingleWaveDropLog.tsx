...
const VoteSummary = ({ log, creditType }: VoteSummaryProps) => (
  <>
    ...
    {log.contents["oldVote"] !== 0 && (
      <span className="tw-whitespace-nowrap tw-text-sm tw-font-medium tw-text-iron-400">
    ...
    <span
      className={`tw-whitespace-nowrap tw-text-sm tw-font-medium ${
...
export const SingleWaveDropLog = ({
  ...
  const avatar = log.invoker.pfp ? (
    <Image
      ...
      width={28}
      height={28}
      className="tw-size-7 tw-flex-shrink-0 tw-rounded-md tw-bg-iron-800 tw-object-contain tw-ring-1 tw-ring-white/10"
    />
  ) : (
    <div className="tw-size-7 tw-flex-shrink-0 tw-rounded-md tw-bg-iron-800 tw-ring-1 tw-ring-white/10" />
  );
  ...
  const handleLink = (
    <UserProfileTooltipWrapper user={log.invoker.handle ?? log.invoker.id}>
      <Link
        ...
        className="tw-group tw-no-underline tw-transition-opacity tw-duration-300 desktop-hover:hover:tw-opacity-80 motion-reduce:tw-transition-none"
      >
        <span
  ...
  return (
    <div className="tw-p-3 tw-transition-colors desktop-hover:hover:tw-bg-white/[0.02] motion-reduce:tw-transition-none">
      {isCompactLayout ? (
        <div className="tw-flex tw-gap-3">
          ...
          <div className="tw-flex tw-min-w-0 tw-flex-1 tw-flex-col">
            ...
            <div className="tw-mt-1 tw-flex tw-flex-wrap tw-items-center tw-gap-x-1.5 tw-gap-y-1.5">
        ...
        <div className="tw-flex tw-items-center tw-justify-between tw-gap-x-2">
          <div className="tw-flex tw-items-center tw-gap-x-2 tw-gap-y-2">
            {avatar}
            <div className="tw-inline-flex tw-flex-wrap tw-items-center tw-gap-x-1 tw-gap-y-1">
              {handleLink}
              <VoteSummary log={log} creditType={creditType} />
            </div>
          </div>
