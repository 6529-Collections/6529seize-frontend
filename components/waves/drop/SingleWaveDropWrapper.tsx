...
interface SingleWaveDropShellProps {
  readonly children: React.ReactNode;
  readonly onClose: () => void;
  readonly setHeaderActionsContainer: (node: HTMLDivElement | null) => void;
  readonly setTrailingContentContainer: (node: HTMLDivElement | null) => void;
}

interface AuthenticatedSingleWaveDropChatControllerProps {
  readonly drop: ApiDrop;
  readonly wave: ApiWave;
  readonly headerActionsContainer: HTMLDivElement | null;
  readonly trailingContentContainer: HTMLDivElement | null;
}

function SingleWaveDropShell({
  children,
  onClose,
  setHeaderActionsContainer,
  setTrailingContentContainer,
}: SingleWaveDropShellProps) {
  return (
    <div className="tw-flex tw-h-full tw-w-full tw-flex-col tw-overflow-hidden tw-overscroll-none tw-bg-iron-950 lg:tw-flex-row">
      <div className="@container tw-relative tw-h-full tw-flex-1 tw-overflow-hidden">
        <header className="tw-absolute tw-left-0 tw-right-3 tw-top-0 tw-z-20 tw-bg-iron-950/55 tw-bg-gradient-to-b tw-from-iron-950/75 tw-via-iron-950/55 tw-to-iron-950/10 tw-px-2 tw-pb-2 tw-pt-[calc(env(safe-area-inset-top,0px)+0.5rem)] tw-backdrop-blur-sm @[640px]:tw-px-8 lg:tw-px-8">
          <div className="tw-flex tw-w-full tw-items-center tw-justify-between">
            ...
            <div ref={setHeaderActionsContainer} className="tw-contents" />
          </div>
        </header>
      ...
      <div ref={setTrailingContentContainer} className="tw-contents" />
    </div>
  );
}

function AuthenticatedSingleWaveDropChatController({
  drop,
  wave,
  headerActionsContainer,
  trailingContentContainer,
}: AuthenticatedSingleWaveDropChatControllerProps) {
  ...
  if (!headerActionsContainer || !trailingContentContainer) {
    return null;
  }
...
export const SingleWaveDropWrapper: React.FC<SingleWaveDropWrapperProps> = ({
  ...
  const [trailingContentContainer, setTrailingContentContainer] =
    useState<HTMLDivElement | null>(null);
  const { isPublicReadOnly } = useWaveViewerMode();

  const handleHeaderActionsContainer = useCallback(
  ...
  const handleTrailingContentContainer = useCallback(
    (node: HTMLDivElement | null) => {
      setTrailingContentContainer(node);
    },
    []
  );
  ...
  return (
    <>
      <SingleWaveDropShell
        onClose={onClose}
        setHeaderActionsContainer={handleHeaderActionsContainer}
        setTrailingContentContainer={handleTrailingContentContainer}
      >
        {children}
      </SingleWaveDropShell>

      {!isPublicReadOnly && (
        <AuthenticatedSingleWaveDropChatController
          drop={drop}
          wave={wave}
