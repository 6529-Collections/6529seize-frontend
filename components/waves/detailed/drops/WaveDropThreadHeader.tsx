...
const WaveDropThreadHeader: React.FC<WaveDropThreadHeaderProps> = ({ rootDropId }) => {
  const { data: drop, isFetching, isRefetching } = useQuery<Drop>({
    ...
    <div className="tw-bg-iron-900 tw-px-4 tw-py-4 tw-mb-2">
      ...
      <p className="tw-text-sm tw-text-iron-400 tw-mn-0">
        ...
        <Link href={`/${drop.author.handle}`} className="tw-no-underline">
          <span className="tw-font-semibold tw-text-iron-50">{drop.author.handle}</span>
        </Link>
      </p>
      <p className="tw-text-xs tw-text-iron-500 tw-mt-1 tw-mb-0">
        {new Date(drop.created_at).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
      </p>
    </div>
  );
};
