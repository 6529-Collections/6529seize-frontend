        ...
        <Link href={`/${voter.profile.handle}`}>
          {voter.profile.pfp ? (
            <img
              ...
              alt={`${voter.profile.handle}'s Profile Picture`}
              className={`tw-h-5 tw-w-5 tw-rounded-md tw-ring-1  tw-bg-iron-900`}
            />
          ) : (
            <div className="tw-h-5 tw-w-5 tw-rounded-md tw-bg-iron-900"></div>
          )}
        </Link>
      </div>
    </Tippy>
