export default function WavesMainSteps() {
  return (
    <div className="tw-col-span-3 tw-pl-24 tw-pt-8">
      <nav aria-label="Progress">
        <div className="tw-flex tw-flex-col">
          <div className="tw-relative tw-mb-11">
            <div
              className="tw-absolute tw-left-3 tw-top-10 tw-h-full tw-rounded-sm tw-w-0.5 tw-bg-primary-500"
              aria-hidden="true"
            ></div>
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-primary-500 tw-bg-primary-600">
                  <svg
                    width="13"
                    height="11"
                    className="tw-text-white"
                    viewBox="0 0 13 11"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M11.0965 0.390037L3.9365 7.30004L2.0365 5.27004C1.6865 4.94004 1.1365 4.92004 0.736504 5.20004C0.346504 5.49004 0.236503 6.00004 0.476503 6.41004L2.7265 10.07C2.9465 10.41 3.3265 10.62 3.7565 10.62C4.1665 10.62 4.5565 10.41 4.7765 10.07C5.1365 9.60004 12.0065 1.41004 12.0065 1.41004C12.9065 0.490037 11.8165 -0.319963 11.0965 0.380037V0.390037Z"
                      fill="currentColor"
                    />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-semibold tw-text-iron-600">
                  Overview
                </span>
              </span>
            </button>
          </div>
          <div className="tw-relative tw-mb-11">
            <div
              className="tw-absolute tw-left-3 tw-top-10 tw-h-full tw-rounded-sm tw-w-0.5 tw-bg-primary-500"
              aria-hidden="true"
            ></div>
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-primary-500 tw-bg-primary-600 tw-shadow-[0_0_0_6px_rgba(41,112,255,0.24)]">
                  <svg
                    width="8"
                    height="8"
                    viewBox="0 0 8 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="4" cy="4" r="4" fill="white" />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-bold tw-text-primary-400">
                  Visibility
                </span>
              </span>
            </button>
          </div>
          <div className="tw-relative tw-mb-11">
            <div
              className="tw-absolute tw-left-3 tw-top-10 tw-h-full tw-rounded-sm tw-w-0.5 tw-bg-iron-800"
              aria-hidden="true"
            ></div>
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-iron-700 tw-bg-iron-900">
                  <svg
                    width="8"
                    height="8"
                    className="tw-text-iron-500"
                    viewBox="0 0 8 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="4" cy="4" r="4" fill="currentColor" />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-semibold tw-text-iron-600">
                  Drops
                </span>
              </span>
            </button>
          </div>
          <div className="tw-relative tw-mb-11">
            <div
              className="tw-absolute tw-left-3 tw-top-10 tw-h-full tw-rounded-sm tw-w-0.5 tw-bg-iron-800"
              aria-hidden="true"
            ></div>
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-iron-700 tw-bg-iron-900">
                  <svg
                    width="8"
                    height="8"
                    className="tw-text-iron-500"
                    viewBox="0 0 8 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="4" cy="4" r="4" fill="currentColor" />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-semibold tw-text-iron-600">
                  Rating
                </span>
              </span>
            </button>
          </div>
          <div className="tw-relative tw-mb-11">
            <div
              className="tw-absolute tw-left-3 tw-top-10 tw-h-full tw-rounded-sm tw-w-0.5 tw-bg-iron-800"
              aria-hidden="true"
            ></div>
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-iron-700 tw-bg-iron-900">
                  <svg
                    width="8"
                    height="8"
                    className="tw-text-iron-500"
                    viewBox="0 0 8 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="4" cy="4" r="4" fill="currentColor" />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-semibold tw-text-iron-600">
                  Period
                </span>
              </span>
            </button>
          </div>
          <div className="tw-relative tw-mb-11">
            <button
              type="button"
              className="tw-bg-transparent tw-p-0 tw-border-none focus:tw-outline-none tw-group tw-relative tw-flex tw-items-center"
            >
              <span className="tw-flex tw-h-9 tw-items-center">
                <span className="tw-relative tw-z-10 tw-flex tw-h-6 tw-w-6 tw-items-center tw-justify-center tw-rounded-full tw-ring-2 tw-ring-iron-700 tw-bg-iron-900">
                  <svg
                    width="8"
                    height="8"
                    className="tw-text-iron-500"
                    viewBox="0 0 8 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <circle cx="4" cy="4" r="4" fill="currentColor" />
                  </svg>
                </span>
              </span>
              <span className="tw-ml-4 tw-flex tw-min-w-0 tw-flex-col">
                <span className="tw-text-base tw-font-semibold tw-text-iron-600">
                  Outcome
                </span>
              </span>
            </button>
          </div>
        </div>
      </nav>
    </div>
  );
}
