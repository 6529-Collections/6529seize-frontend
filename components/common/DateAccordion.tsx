...
interface DateAccordionProps {
  ...
  // Opt-in: make the ENTIRE header row the toggle target (one big button)
  // instead of just the chevron+title. Only safe when titleActions holds no
  // interactive elements (a button can't nest interactive content) — e.g. the
  // Saved Drafts card, whose only action is a static count badge. The date
  // steps leave this off because their titleActions is a tooltip button.
  readonly fullHeaderToggle?: boolean | undefined;
}
...
export default function DateAccordion({
  ...
  const chevron = (
    <m.span
      // Match the app-wide chevron convention: points down when collapsed,
      // rotates up when expanded (as in GrantTokensDisclosure,
      // TimelineToggleHeader, the dropdowns).
  ...
  const actionsNode = titleActions !== undefined && titleActions !== null && (
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      >
        {fullHeaderToggle ? (
          // Whole-row toggle: safe because the caller guarantees non-interactive
          // header content (no nested buttons/links).
          <button
            type="button"
            onClick={onToggle}
            ...
            className="tw-flex tw-h-16 tw-w-full tw-cursor-pointer tw-items-center tw-gap-x-3 tw-rounded-xl tw-border-0 tw-bg-transparent tw-px-5 tw-text-left tw-transition-colors tw-duration-200 focus-visible:tw-outline-none focus-visible:tw-ring-2 focus-visible:tw-ring-primary-400"
          >
            {chevron}
            {titleNode}
            {actionsNode}
