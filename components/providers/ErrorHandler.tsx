...
const REACT_ERROR_CODES: Record<string, string> = {
  ...
  "185": "Hydration failed because the initial UI does not match what was rendered on the server.",
...
function decodeReactError(message: string): { decoded: string; code?: string } {
  ...
  if (match) {
    const code = match[1];
    const decodedMessage = REACT_ERROR_CODES[code] || `Unknown React error code: ${code}`;
    return {
  ...
  const enhanced = lines.map((line, index) => {
    ...
    if (trimmed.startsWith("at ") || trimmed.includes("@")) {
      ...
      if (atMatch) {
        ...
        const isReactFile = file.includes("react") || file.includes("react-dom");
        const marker = isReactFile ? " [React Internal]" : "";
        return `  at ${funcName} (${file}:${lineNum}:${colNum})${marker}`;
      }
      
      const atSimpleMatch = trimmed.match(/at\s+(.+?)\s+@\s+(.+?):(\d+):(\d+)/);
      if (atSimpleMatch) {
    ...
    if (trimmed.match(/^\w+\s+@\s+/)) {
      ...
      if (parts.length === 2) {
        ...
        const isReactFile = location.includes("react") || location.includes("react-dom");
        const marker = isReactFile ? " [React Internal - use source maps in DevTools]" : "";
  ...
  useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      ...
      if (error) {
        ...
        console.error("   to see original file names and line numbers mapped from the minified code.");
      }
