...
export default function GroupCreateXtdhGrant({
  ...
  const [isManuallyEnabled, setIsManuallyEnabled] = useState(hasSelectedGrant);
  const [lookupGrantId, setLookupGrantId] = useState<string | null>(
    hasSelectedGrant ? normalizedGrantId : null
  );
  const [isModalOpen, setIsModalOpen] = useState(false);
  const isEnabled = isManuallyEnabled || hasSelectedGrant;
  ...
  const { grant, isFetching, isError, errorMessage } = useXtdhGrantQuery({
    ...
    enabled: isEnabled && !!lookupGrantId,
  });
  ...
  const showLookupError = Boolean(isEnabled && lookupGrantId && isError);

  const onEnabledChange = (nextEnabled: boolean) => {
    setIsManuallyEnabled(nextEnabled);
    if (!nextEnabled) {
  ...
  return (
    <div className="tw-rounded-xl tw-border tw-border-solid tw-border-iron-800 tw-bg-iron-950 tw-p-3 tw-shadow sm:tw-p-5">
      <div className="tw-flex tw-flex-wrap tw-items-center tw-justify-between tw-gap-3">
        ...
        <label
          htmlFor="group-create-beneficiary-grant-toggle"
        ...
        >
          ...
          <div
            className={`tw-rounded-full tw-bg-gradient-to-b tw-p-[1px] ${
              isEnabled ? "tw-from-primary-300" : "tw-from-iron-600"
            }`}
          >
            <input
              id="group-create-beneficiary-grant-toggle"
              type="checkbox"
              checked={isEnabled}
              onChange={() => onEnabledChange(!isEnabled)}
              className="tw-sr-only"
            />
            <span
              className={`tw-relative tw-flex tw-h-6 tw-w-11 tw-flex-shrink-0 tw-rounded-full tw-border-2 tw-border-transparent tw-transition-colors tw-duration-200 ${
                isEnabled ? "tw-bg-primary-500" : "tw-bg-iron-700"
              }`}
              role="switch"
            ...
            >
              <span
                aria-hidden="true"
                className={`tw-pointer-events-none tw-inline-block tw-h-5 tw-w-5 tw-transform tw-rounded-full tw-bg-iron-50 tw-shadow tw-transition tw-duration-200 ${
                  isEnabled ? "tw-translate-x-5" : "tw-translate-x-0"
                }`}
              ></span>
            </span>
          </div>
          ...
          {showNonGrantedWarning && (
            <div className="tw-mt-3 tw-rounded-lg tw-border tw-border-solid tw-border-amber-300/30 tw-bg-amber-300/10 tw-p-3">
              <p className="tw-m-0 tw-text-xs tw-font-medium tw-text-amber-300">
                Selected grant status is not GRANTED. This filter is still
                allowed and will be submitted.
              </p>
            </div>
          )}
        </>
