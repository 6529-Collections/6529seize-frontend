...
export default function WalletCheckerComponent(
  ...
  function fetchDelegations(address: string) {
    const url = `${env.API_ENDPOINT}/api/delegations/${address}`;
  ...
  function fetchConsolidatedWallets(address: string) {
    const url = `${env.API_ENDPOINT}/api/consolidations/${address}`;
    fetchUrl(url).then((response: DBResponse) => {
  ...
  function fetchConsolidations(address: string) {
    const url = `${env.API_ENDPOINT}/api/consolidations/${address}?show_incomplete=true`;
    fetchUrl(url).then((response1: DBResponse) => {
      if (response1.data.length > 0) {
        ...
        const newUrl = `${env.API_ENDPOINT}/api/consolidations/${newWallet}?show_incomplete=true`;
