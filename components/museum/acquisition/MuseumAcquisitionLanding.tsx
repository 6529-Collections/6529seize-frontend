...
export function MuseumAcquisitionMediaCard({
  ...
  const acquisitionHref = museumAcquisitionHref(acquisition.slug);
  const textHref = captionHref ?? acquisitionHref;
  const creditLine = record.media?.creditLine;
  const sourceHref =
  ...
  return (
    <figure
      className={`group tw-m-0 tw-min-w-0 tw-overflow-hidden tw-rounded-xl tw-border tw-border-solid tw-border-iron-800 tw-bg-iron-950 ${featured ? "tw-shadow-[0_24px_80px_rgba(0,0,0,0.35)]" : ""}`}
    >
      <div className="tw-block">
        <div
          className="tw-overflow-hidden tw-bg-iron-950"
