...
const getNavClassName = ({ hidden }: { readonly hidden: boolean }) =>
  `${getHiddenStyle(hidden)} tw-pointer-events-none tw-fixed tw-inset-x-0 tw-bottom-0 tw-z-50 tw-flex tw-justify-center tw-px-4 tw-pb-[max(calc(max(env(safe-area-inset-bottom,0px),var(--safe-area-inset-bottom,0px))-0.875rem),0px)] tw-transition-[opacity,transform] tw-duration-200 tw-ease-out motion-reduce:tw-transition-none`;
