...
const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  return (
    <nav
      ref={setMobileNavRef}
      className={`${paddingClass} ${hiddenStyle} tw-fixed tw-bottom-0 tw-left-0 tw-z-50 tw-w-full tw-border-x-0 tw-border-b-0 tw-border-t tw-border-solid tw-border-iron-900 tw-bg-black tw-shadow-inner tw-transition-[opacity,transform] tw-duration-75`}
    >
      <div className="tw-h-full">
        <ul className="tw-mx-auto tw-flex tw-h-full tw-pl-[env(safe-area-inset-left,0px)] tw-pr-[env(safe-area-inset-right,0px)] md:tw-max-w-2xl">
          {navItems.map((item) => (
            <li
              key={item.name}
              className={`tw-flex tw-flex-1 tw-items-end tw-justify-center ${
