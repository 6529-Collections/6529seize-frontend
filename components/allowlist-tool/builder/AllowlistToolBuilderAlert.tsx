...
export default function AllowlistToolBuilderAlert() {
  ...
  const [allowlistStatus, setAllowlistStatus] =
    useState<AllowlistRunStatus | null>(null);

  useEffect(() => {
    if (!allowlist?.activeRun?.status) return;
    if (allowlist.activeRun.status !== AllowlistRunStatus.FAILED) {
      setShowAlert(false);
    } else if (allowlistStatus !== allowlist.activeRun.status) {
      setShowAlert(true);
    }
    setAllowlistStatus(allowlist.activeRun.status);
  }, [allowlist?.activeRun?.status, allowlistStatus]);
  return (
          ...
          <div className="tw-relative tw-rounded-lg tw-bg-[#282026]/80 tw-ring-1 tw-ring-[#632c28] tw-p-4">
            ...
            <div className="tw-absolute tw-right-4 tw-top-2">
              <button
                ...
                type="button"
                className="tw-p-2.5 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-bg-transparent tw-border-0 focus:tw-outline-none tw-transition tw-duration-300 tw-ease-out"
              >
                <span className="tw-sr-only">Close</span>
                <svg
                  className="tw-h-5 tw-w-5 tw-text-error"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
