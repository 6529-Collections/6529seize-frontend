...
import type { XtdhReceivedError, XtdhReceivedNftsResponse } from "@/types/xtdh";
import {
...
type NftSortField = (typeof NFT_SORT_FIELDS)[number];

type SortDirection = "asc" | "desc";

function parseSortField(value: string | null): NftSortField | null {
  if (!value) return "xtdh_rate";
  const normalized = value.toLowerCase();
  return NFT_SORT_FIELDS.find((field) => field === normalized) ?? null;
}
...
export async function GET(
  ...
  if (Math.random() < SERVER_ERROR_PROBABILITY) {
    const error: XtdhReceivedError = {
      ...
      message: "Failed to fetch xTDH received NFTs",
      statusCode: 500,
    };
  ...
  const nfts = buildNfts(filteredTokens);

  const sortedNfts = [...nfts].sort((a, b) => {
    switch (sort) {
      case "xtdh_rate":
        return dir === "asc" ? a.xtdhRate - b.xtdhRate : b.xtdhRate - a.xtdhRate;
      case "total_received":
        return dir === "asc"
  ...
  const paginatedNfts =
    start >= 0 && start < sortedNfts.length
      ? sortedNfts.slice(start, start + pageSize)
      : [];

  const response: XtdhReceivedNftsResponse = {
    nfts: paginatedNfts,
    totalCount,
    page,
    pageSize,
