...
const getEstimatedTextWidth = (value: string): number =>
  [...value].reduce(
    (width, character) =>
      width + getCharacterWidthFactor(character) * DESCRIPTION_FONT_SIZE,
    0
  );

const fitsDescriptionLine = (value: string): boolean =>
  getEstimatedTextWidth(value) <= DESCRIPTION_WRAP_WIDTH;
...
const appendEllipsis = (value: string): string => {
  ...
  if (fitsDescriptionLine(`${value}${suffix}`)) {
    return `${value}${suffix}`;
  }

  let truncated = value.trimEnd();
  while (truncated.length > 0 && !fitsDescriptionLine(`${truncated}${suffix}`)) {
...
const truncateWordToDescriptionLine = (word: string): string => {
  let truncated = word;
  while (truncated.length > 0 && !fitsDescriptionLine(truncated)) {
...
const getDescriptionLines = (
  ...
  for (const word of words) {
    ...
    if (fitsDescriptionLine(nextLine)) {
    ...
    if (currentLine) {
      ...
      lines.push(truncateWordToDescriptionLine(word));
    }
  ...
  if (currentLine && lines.length < DESCRIPTION_MAX_LINES) {
    lines.push(currentLine);
  }

  const consumedText = lines.join(" ");
  const shouldTruncate = consumedText.length < normalized.length;
  if (!shouldTruncate || lines.length === 0) {
    return lines;
  }

  const lastLineIndex = lines.length - 1;
  lines[lastLineIndex] = appendEllipsis(lines[lastLineIndex]);
...
const getStats = (profile: ProfileWithOptionalRates | undefined) => {
  ...
  const stats = [
    {
      label: "TDH",
      value: formatNumber(profile?.tdh),
      rate: formatSignedNumber(profile?.tdh_rate),
    },
    {
      label: "NIC",
      value: formatNumber(profile?.cic),
      rate: null,
    },
    {
      label: "Rep",
      value: formatNumber(profile?.rep),
...
const ArtistActivityBadge = ({ type }: { readonly type: ActivityBadgeType }) => {
...
const getActivityBadgeType = (
  profile: ProfileWithOptionalRates | undefined
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <CicBadge cic={profile?.cic ?? 0} />
          {profile?.level !== null && profile?.level !== undefined ? (
            <LevelBadge level={profile.level} />
          ) : null}
          {activityBadgeType ? (
            <ArtistActivityBadge type={activityBadgeType} />
          ) : null}
        </div>
        <div
          style={{
