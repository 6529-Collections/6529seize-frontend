...
const WAVES_ENTRY_STORAGE_PREFIX = "join:entered-waves:";
  ...
  return (
    <header className="tw-grid tw-gap-6 lg:tw-grid-cols-[minmax(0,1fr)_360px] lg:tw-items-end">
      <div className="tw-space-y-3">
        ...
        <p className="tw-max-w-3xl tw-text-base tw-leading-7 tw-text-iron-300 sm:tw-text-lg">
      ...
      >
        <div className="tw-flex tw-items-center tw-justify-between tw-gap-4">
          <p className="tw-min-w-0 tw-text-sm tw-font-medium tw-text-iron-400">
            {m(locale, "join6529.progress.label")}
          </p>
          <span className="tw-shrink-0 tw-rounded-full tw-border tw-border-primary-400/25 tw-bg-primary-400/10 tw-px-3 tw-py-1 tw-text-xs tw-font-semibold tw-text-primary-300">
        ...
        <progress
          ...
          className="tw-mt-5 tw-block tw-h-2 tw-w-full tw-appearance-none tw-overflow-hidden tw-rounded-full tw-bg-white/10 [&::-moz-progress-bar]:tw-rounded-full [&::-moz-progress-bar]:tw-bg-primary-400 [&::-webkit-progress-bar]:tw-rounded-full [&::-webkit-progress-bar]:tw-bg-white/10 [&::-webkit-progress-value]:tw-rounded-full [&::-webkit-progress-value]:tw-bg-primary-400"
          max={totalSteps}
...
function CurrentStepPanel({
  ...
  return (
    <section className="tw-rounded-lg tw-border tw-border-white/10 tw-bg-white/[0.04] tw-p-5">
      <p className="tw-text-xs tw-font-semibold tw-uppercase tw-tracking-[0.16em] tw-text-primary-300">
    ...
    <section className="tw-space-y-4">
      <div className="tw-flex tw-flex-col tw-gap-2 sm:tw-flex-row sm:tw-items-end sm:tw-justify-between">
        <div>
          ...
          <p className="tw-mt-2 tw-max-w-2xl tw-text-sm tw-leading-6 tw-text-iron-400">
  ...
  return (
    <li
      ...
      className={cx(
        ...
        status === "current" && "tw-border-primary-400/30 tw-bg-primary-400/[0.07]",
    ...
    >
      <div
        className={cx(
          ...
          status === "current" &&
            "tw-border-primary-400/30 tw-bg-primary-400/10 tw-text-primary-300",
          status === "pending" &&
            "tw-border-white/10 tw-bg-black/30 tw-text-iron-400"
        )}
      >
        {step.complete ? (
        ...
        <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
          ...
          <span
            className={cx(
              ...
              status === "complete" && "tw-bg-primary-400/10 tw-text-primary-300",
              status === "current" && "tw-bg-primary-400/10 tw-text-primary-300",
              status === "pending" && "tw-bg-white/10 tw-text-iron-300"
            )}
          >
            {statusLabel}
          </span>
        </div>
        <p className="tw-text-sm tw-leading-6 tw-text-iron-400">{step.body}</p>
      </div>
    </li>
  );
}
...
function ActionTileCard({ tile }: { readonly tile: ActionTile }) {
  ...
  return (
    <Link
      className="tw-group tw-flex tw-min-h-[150px] tw-flex-col tw-justify-between tw-rounded-lg tw-border tw-border-white/10 tw-bg-iron-950 tw-p-4 tw-text-inherit tw-no-underline tw-transition hover:tw-border-primary-400/35 hover:tw-bg-primary-400/[0.06] hover:tw-text-inherit hover:tw-no-underline focus:tw-no-underline focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-primary-400/60"
      href={tile.href}
    >
      <div className="tw-flex tw-items-start tw-justify-between tw-gap-4">
        <div className="tw-flex tw-h-10 tw-w-10 tw-items-center tw-justify-center tw-rounded-lg tw-border tw-border-white/10 tw-bg-white/[0.04] tw-text-primary-300">
          <Icon aria-hidden="true" className="tw-h-5 tw-w-5" />
        </div>
        <ArrowRightIcon
          aria-hidden="true"
      ...
      <div className="tw-mt-5 tw-space-y-2">
        ...
        <p className="tw-text-sm tw-leading-6 tw-text-iron-400">{tile.body}</p>
      </div>
    </Link>
  );
}
