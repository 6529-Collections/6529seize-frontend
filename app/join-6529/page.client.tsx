...
const getProfileLabel = (
  ...
  return m("join6529.status.missing");
};

const isPublicWaveDrop = (drop: ApiDrop): boolean => {
  const wave = drop.wave;
  return (
    !wave.visibility_group_id && !wave.chat_group_id && !wave.identity_wave
  );
};

export default function Join6529PageClient() {
  useSetTitle(m("join6529.metadata.title"));
  ...
  const wavesEntryStorageKey = profileStorageKey
    ...
    : null;
  const dropAuthor =
    connectedProfile?.handle ?? connectedProfile?.normalised_handle ?? null;
  const contextProfile = connectedProfile?.handle ?? null;
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (contextProfile) {
        params["context_profile"] = contextProfile;
      }
  ...
  const hasFirstPublicMessage = Boolean(
    recentProfileDrops?.some(isPublicWaveDrop)
  );
  ...
  const steps = useMemo<JourneyStep[]>(
    () => [
      {
        id: "wallet",
        title: m("join6529.steps.wallet.title"),
        body: m("join6529.steps.wallet.body"),
        icon: WalletIcon,
        complete: hasActiveWalletAddress,
      },
      {
        id: "profile",
        title: m("join6529.steps.profile.title"),
        body: m("join6529.steps.profile.body"),
        icon: UserCircleIcon,
        complete: hasProfile,
      },
      {
        id: "pfp",
        title: m("join6529.steps.pfp.title"),
        body: m("join6529.steps.pfp.body"),
        icon: PhotoIcon,
      ...
      {
        id: "waves",
        title: m("join6529.steps.waves.title"),
        body: m("join6529.steps.waves.body"),
        icon: ChatBubbleLeftRightIcon,
        complete: hasEnteredWaves,
      },
      {
        id: "message",
        title: m("join6529.steps.message.title"),
        body: m("join6529.steps.message.body"),
  ...
  const currentPanel = useMemo<CurrentPanelContent>(() => {
    if (!hasActiveWalletAddress) {
      return {
        title: m("join6529.current.wallet.title"),
        body: m("join6529.current.wallet.body"),
        action: {
          kind: "button",
          label: m("join6529.action.connect"),
          busyLabel: m("join6529.action.connecting"),
    ...
    if (!hasValidWalletAuth) {
      return {
        title: m("join6529.current.auth.title"),
        body: m("join6529.current.auth.body"),
        action: {
          kind: "button",
          label: m("join6529.action.sign"),
          busyLabel: m("join6529.action.signing"),
    ...
    if (fetchingProfile) {
      return {
        title: m("join6529.current.profile.loadingTitle"),
        body: m("join6529.current.profile.loadingBody"),
      };
    }

    if (!hasProfile) {
      return {
        title: m("join6529.current.profile.title"),
        body: m("join6529.current.profile.body"),
        action: {
          kind: "link",
          label: m("join6529.action.createProfile"),
          href: profileHref,
        },
      };
    }
    ...
    if (!hasEnteredWaves) {
      return {
        title: m("join6529.current.waves.title"),
        body: m("join6529.current.waves.body"),
        action: {
          kind: "link",
          label: m("join6529.action.openWaves"),
          href: WAVES_HREF,
          onClick: markWavesEntered,
        },
      };
    }
    ...
    if (!hasFirstPublicMessage) {
      return {
        title: m("join6529.current.message.title"),
        body: m("join6529.current.message.body"),
        action: {
          kind: "link",
          label: m("join6529.action.openWaves"),
          href: WAVES_HREF,
    ...
    if (!hasProfileImage) {
      return {
        title: m("join6529.current.pfp.title"),
        body: m("join6529.current.pfp.body"),
        action: {
          kind: "link",
          label: m("join6529.action.openProfile"),
          href: profileHref,
        },
      };
    }
    ...
    return {
      title: m("join6529.current.complete.title"),
      body: m("join6529.current.complete.body"),
      action: {
        kind: "link",
        label: m("join6529.action.createWave"),
  ...
  const actionTiles = useMemo<ActionTile[]>(
    () => [
      {
        title: m("join6529.things.explore.title"),
        body: m("join6529.things.explore.body"),
        href: WAVES_HREF,
        icon: ChatBubbleLeftRightIcon,
        onClick: markWavesEntered,
      },
      {
        title: m("join6529.things.help.title"),
        body: m("join6529.things.help.body"),
        href: WAVES_HREF,
        icon: QuestionMarkCircleIcon,
        onClick: markWavesEntered,
      },
      {
        title: m("join6529.things.subscriptions.title"),
        body: m("join6529.things.subscriptions.body"),
        href: subscriptionsHref,
        icon: BookOpenIcon,
      },
      {
        title: m("join6529.things.memes.title"),
        body: m("join6529.things.memes.body"),
        href: WAVES_HREF,
        icon: SparklesIcon,
        onClick: markWavesEntered,
      },
      {
        title: m("join6529.things.delegation.title"),
        body: m("join6529.things.delegation.body"),
        href: "/delegation/delegation-center",
        icon: LightBulbIcon,
      },
      {
        title: m("join6529.things.createWave.title"),
        body: m("join6529.things.createWave.body"),
        href: CREATE_WAVE_HREF,
        icon: PlusCircleIcon,
      },
    ],
    [markWavesEntered, subscriptionsHref]
  );
      ...
      <div className="tw-mx-auto tw-flex tw-w-full tw-max-w-[1180px] tw-flex-col tw-gap-8 tw-px-4 tw-py-6 sm:tw-px-6 lg:tw-px-8 lg:tw-py-10">
        ...
        <div className="tw-grid tw-gap-6 xl:tw-grid-cols-[minmax(0,1fr)_380px]">
          <section className="tw-min-w-0">
            <ol
              aria-label={m("join6529.steps.ariaLabel")}
          ...
          <aside className="tw-flex tw-flex-col tw-gap-4">
            ...
            <section className="tw-rounded-lg tw-border tw-border-white/10 tw-bg-white/[0.03] tw-p-5">
              <h2 className="tw-text-sm tw-font-semibold tw-uppercase tw-tracking-[0.16em] tw-text-slate-300">
                {m("join6529.status.heading")}
              </h2>
              <div className="tw-mt-4 tw-divide-y tw-divide-white/10">
                <StateRow
                  label={m("join6529.status.wallet")}
                  value={
                    hasActiveWalletAddress
                      ? getProfileLabel(null, address)
                      : m("join6529.status.notConnected")
                  }
                  good={hasActiveWalletAddress}
                />
                <StateRow
                  label={m("join6529.status.profile")}
                  value={
                    hasProfile
                      ? getProfileLabel(connectedProfile, address)
                      : m("join6529.status.missing")
                  }
                  good={hasProfile}
                />
                <StateRow
                  label={m("join6529.status.pfp")}
                  value={
                    hasProfileImage
                      ? m("join6529.status.ready")
                      : m("join6529.status.missing")
                  }
                  good={hasProfileImage}
                />
                <StateRow
                  label={m("join6529.status.message")}
              ...
              {didPublicMessageCheckFail && (
                <p className="tw-mt-4 tw-text-xs tw-leading-5 tw-text-amber-200">
                  {m("join6529.status.checkFailed")}
                </p>
              )}
            </section>
          </aside>
...
function JourneyStepRow({
  ...
  const statusLabels: Record<StepStatus, string> = {
    complete: m("join6529.progress.done"),
    current: m("join6529.progress.current"),
    pending: m("join6529.progress.pending"),
  };
...
function CurrentAction({ action }: { readonly action: CurrentPanelAction }) {
  ...
  if (action.kind === "link" && action.href) {
    const onClickProps = action.onClick ? { onClick: action.onClick } : {};
    return (
      <Link className={className} href={action.href} {...onClickProps}>
        {content}
      </Link>
    );
  }
...
function ActionTileCard({ tile }: { readonly tile: ActionTile }) {
  ...
  const onClickProps = tile.onClick ? { onClick: tile.onClick } : {};
  return (
    <Link
...
function getPublicMessageStatus({
  ...
  if (hasFirstPublicMessage) {
    return m("join6529.status.detected");
  }
  if (checkingPublicMessage) {
    return m("join6529.status.checking");
  }
  return m("join6529.status.notDetected");
}
