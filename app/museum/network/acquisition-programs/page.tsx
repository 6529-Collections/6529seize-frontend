...
export default async function MuseumAcquisitionProgramsPage() {
  ...
  if (programs.length === 0 && view === null) return <MuseumPublicationUnavailable />;

  return (
    <section>
      <MuseumSectionHeading
        eyebrow={t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.eyebrow")}
        title={t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.title")}
        description={t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.description")}
      />
      <ul className="tw-m-0 tw-list-none tw-border-y tw-border-solid tw-border-iron-800 tw-p-0">
        {programs.map((program) => (
          <li key={program.id} className="tw-border-b tw-border-solid tw-border-iron-800 last:tw-border-b-0">
            <Link
            ...
            >
              <span>
                ...
                <span className="tw-mt-1 tw-block tw-text-sm tw-text-iron-400">
                  {program.acquisitionIds.length} {t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.acquisitions").toLocaleLowerCase()}
                </span>
              </span>
              <span className="tw-text-sm tw-text-primary-300">{t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.view")}</span>
            </Link>
          </li>
        ))}
        {programs.length === 0 && view?.programs.map((program) => (
          <li key={program.programId} className="tw-border-b tw-border-solid tw-border-iron-800 last:tw-border-b-0">
            <Link
            ...
            >
              <span className="tw-text-lg tw-font-semibold tw-text-iron-50">{program.title}</span>
              <span className="tw-text-sm tw-text-primary-300">{t(DEFAULT_LOCALE, "museum.network.acquisitionPrograms.view")}</span>
            </Link>
          </li>
        ))}
