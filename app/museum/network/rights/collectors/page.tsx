...
export async function renderMuseumRightsForCollectorsPage() {
  const publicationState = await getMuseumPublicationState();
  const publication = publicationState.publication;
  if (publication === null) {
    return <MuseumPublicationUnavailable />;
  }
  const document = publication.rightsHandbook.collectorGuide;
    ...
    <article className="tw-min-w-0">
      ...
      <header className="tw-mt-6 tw-max-w-5xl">
        ...
        <p className="tw-m-0 tw-mt-6 tw-max-w-4xl tw-text-lg tw-leading-8 tw-text-iron-200">
          {t(DEFAULT_LOCALE, "museum.network.rights.collectorGuideDescription")}
        </p>
      </header>
...
export default function MuseumRightsForCollectorsLegacyPage() {
  permanentRedirect("/museum/network/research/rights/collectors");
}
