...
for (const source of [
  ...
  test(`rejects unscoped locator: ${source}`, async () => {
    ...
    assert.ok(
      messages.every(
        (message) => message.ruleId === "e2e-selectors/prefer-accessible"
      )
    );
  });
}
...
test("checks helpers, TSX, JS and the e2e directory", async () => {
  ...
  assert.equal(
    (
      await lint(
        'const view = <div />; page.locator(".new")',
        "tests/fixture.tsx"
      )
    ).length,
    1
...
test("baseline allows exact unchanged calls after line shifts and CRLF changes", async () => {
  ...
  assert.deepEqual(
    newViolations(current, await lint(current), base, await lint(base)),
    []
  );
});
...
test("Git baseline checks committed, unstaged and untracked files and shrinks with the base", async (t) => {
  ...
  write("tests/old.spec.ts", 'page.locator(".old");');
  assert.equal(
    (await lintSelectors({ cwd, baseRef: "clean-base" })).failures.length,
    1
  );
