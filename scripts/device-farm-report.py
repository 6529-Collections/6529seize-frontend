...
def summarize(folder, farm_result, platform, expected_devices):
    ...
    complete = count > 0 and len(results) == count and len({device for device, _ in results}) == count
    ...
    for device, result in results:
        ...
        if result is None:
            lines.append(f"| {device} | Evidence unavailable; tests not verified | — | — | — |")
            continue
    ...
    passed = farm_result == "PASSED" and complete and all(clean_pass(r) for _, r in results)
    return "\n".join(lines) + "\n", passed
