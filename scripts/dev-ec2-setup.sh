...
create_env_file() {
  ...
  # Load existing values (if any) so we can prefill prompts
  declare -A EXISTING
  if [[ -f "$env_file" ]]; then
    # Only treat it as “existing config” if it has at least one key=value
    if grep -Eq '^[A-Za-z_][A-Za-z0-9_]*=' "$env_file"; then
      while IFS= read -r line; do
        ...
        IFS='=' read -r k v <<< "$line"
        k="${k//[[:space:]]/}"
        # trim surrounding quotes on value
        v="${v%%[[:space:]]}"
        v="${v%\"}"; v="${v#\"}"
        v="${v%\'}"; v="${v#\'}"
        [[ -n "$k" ]] && EXISTING["$k"]="$v"
      done < "$env_file"
      color yellow ".env exists; we'll prefill prompts from it, then rewrite a clean .env."
    else
      color yellow ".env exists but has no key=value lines; will recreate from .env.sample."
    fi
  else
  ...
  # Build a new .env
  tmp_env="$(mktemp)"
  echo "# Autogenerated .env file $(date -u +'%Y-%m-%dT%H:%M:%SZ')" > "$tmp_env"

  # Iterate sample lines and prompt
  while IFS= read -r line || [[ -n "$line" ]]; do
    # Preserve comments/blank lines from sample
    if [[ -z "${line//[[:space:]]/}" || "$line" =~ ^[[:space:]]*# ]]; then
      echo "$line" >> "$tmp_env"
      continue
    fi

    # Strip inline comments on sample line: KEY=VAL # comment
    local raw="${line%%#*}"

    # Must look like KEY=VAL (no export, etc.)
    if ! [[ "$raw" =~ ^[[:space:]]*([A-Za-z_][A-Za-z0-9_]*)[[:space:]]*=(.*)$ ]]; then
      # copy through unusual lines verbatim to be safe
      echo "$line" >> "$tmp_env"
      continue
    fi
    ...
    # Trim whitespace
    key="${key//[[:space:]]/}"
    # normalize default: remove surrounding spaces and quotes
    def="${def%%[[:space:]]*}"
    def="${def%\"}"; def="${def#\"}"
    def="${def%\'}"; def="${def#\'}"

    # Choose current value precedence: EXISTING > default-from-sample
    ...
    # Write key=value (quote only if contains spaces or # or leading/trailing spaces)
