...
function toSnapshotAsset(
  ...
  const dimensions = fullImage
    ? getOriginalImageDimensions(asset.metadata)
    : {};
...
function getOriginalImageDimensions(metadata: unknown): {
  width?: number;
  height?: number;
} {
  if (
    ...
    metadata === null ||
    !("image_details" in metadata)
  )
    return {};
  const details = metadata.image_details;
  if (
    ...
    return {};
  const width = positiveInteger(details.width);
  const height = positiveInteger(details.height);
