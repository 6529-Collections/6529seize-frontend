...
From the **repository root**, use the pnpm scripts below (they run `build:env-schema` first, then `scripts/export-mint-page.cjs`). Deploy is manual: run the export script locally (or in CI you wire up yourself), then use `pnpm run export-mint-page:*:sync` when you want S3 + CloudFront.

## Prerequisites

- Node / pnpm (same as the main app)
...
| ------- | ------------ | ------- | ------------------------ |
| `pnpm run export-mint-page` | Prod | No | No |
| `pnpm run export-mint-page:sync` | Prod | Yes | Yes: distribution id from `aws cloudfront list-distributions` with `--query` on `Aliases.Items` containing `thememes.6529.io` (same as bucket hostname) |
| `pnpm run export-mint-page:test` | Test | No | No |
| `pnpm run export-mint-page:test:sync` | Test | Yes | Same JMESPath lookup for `thememestest.6529.io` |

### Passing flags manually

pnpm forwards script arguments after `--`:

```bash
pnpm run export-mint-page -- --test --sync
```
