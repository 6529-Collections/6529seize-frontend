...
description: Maintain user-facing Markdown documentation in `/docs` using one commit as discovery input, while actively curating global docs information architecture. Organize docs into clear area-based folders, maintain navigable indexes, and keep content focused on current user-visible behavior only.
---

# Commit Docs Updater

Keep `/docs` continuously accurate as a site guide and navigable information architecture. A commit is discovery input only, never part of docs narration.
...
- Commit-guided, not commit-written:
  ...
  - Never include commit hashes, commit subjects, PR/change-log framing, or "what changed in commit" sections in `/docs`.
- User-facing only:
...
- Information architecture is part of the job:
  - Reorganize `/docs` when structure is flat, fuzzy, or hard to navigate.
...
- Scale rules:
  - Root `docs/README.md` must stay concise (target `<=20` task links in `I am trying to...`).
...
- If no user-visible impact exists, make no `/docs` edits and report that explicitly.

4. Assess global docs architecture.
- Read `references/doc-structure-strategy.md`.
- Inspect whether `/docs` is discoverable from root `README.md` and area/subarea indexes.
...
- Root `docs/README.md` must be a usable start point for browsing.
...
- Avoid redundant area prefixes inside area/subarea folders.
  - Example: use `docs/waves/create/feature-dates-step.md`, not `feature-wave-create-dates-step.md`.
