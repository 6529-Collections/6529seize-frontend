...
export function useWave(
  ...
  const enableParticipationTimer =
    opts.enableTimers !== false && opts.enableParticipationTimer !== false;
  const enableVotingTimer =
    opts.enableTimers !== false && opts.enableVotingTimer !== false;

  // Basic state information - internally using the enum for compatibility
  const [votingPhaseInternal, setVotingPhaseInternal] =
    useState<WaveVotingState>(WaveVotingState.NOT_STARTED);
  ...
  const mapVotingPhase = (state: WaveVotingState): VotingPhase => {
    switch (state) {
      case WaveVotingState.NOT_STARTED:
        return "NOT_STARTED";
      case WaveVotingState.ONGOING:
        return "ONGOING";
      case WaveVotingState.ENDED:
        return "ENDED";
      default:
        return "NOT_STARTED";
    }
  ...
  const hasFirstDecisionPassed = firstDecisionTime
    ? Time.currentMillis() >= firstDecisionTime
    : false;
  ...
  useEffect(() => {
    ...
    if (!minTime && !maxTime) {
      ...
      newVotingPhase =
        now <= maxTime ? WaveVotingState.ONGOING : WaveVotingState.ENDED;
    } else if (minTime !== null && !maxTime) {
      newVotingPhase =
        now < minTime ? WaveVotingState.NOT_STARTED : WaveVotingState.ONGOING;
