...
export const formatWaveName = ({ name, type }: FormatWaveNameArgs): string => {
  ...
  const candidateAddress = name.slice(
    addressStart,
    addressStart + ADDRESS_LENGTH
  );
...
export const formatDirectMessageWaveName = (name: string): string =>
  formatWaveName({ name, type: ApiWaveType.Chat });
