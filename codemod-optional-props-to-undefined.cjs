...
function collectEdits(sourceFile) {
  ...
  function maybeRewrite(node) {
    if (
      ...
      const questionStart = node.questionToken.getStart(sourceFile);
      const questionEnd = node.questionToken.getEnd();
      edits.push({ start: questionStart, end: questionEnd, text: "" });
      ...
      const newTypeNode = ts.factory.createUnionTypeNode([
        node.type,
        undefinedType,
      ]);
      const newTypeText = printer.printNode(
