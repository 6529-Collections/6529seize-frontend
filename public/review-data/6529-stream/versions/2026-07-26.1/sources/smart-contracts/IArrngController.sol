...
interface IArrngController {
    ...
    event ArrngRefundInsufficientTokenForGas(address indexed caller, uint256 requestId);
     ...
     *
     */
    function withdrawERC721(address erc721Address_, uint256[] memory tokenIDs_) external;
     ...
     */
    function requestRandomWords(uint256 numberOfNumbers_)
        external
        payable
        returns (uint256 uniqueID_);
     ...
     * @return uniqueID_ : unique ID for this request
     */
    function requestRandomWords(uint256 numberOfNumbers_, address refundAddress_)
        external
        payable
        returns (uint256 uniqueID_);
